...
class Harvester extends BaseAligner<ThreddsParams> implements IHarvester<HarvestResult> {
    ...
    private static final Namespace invCatalog = Namespace.getNamespace("http://www.unidata.ucar.edu/namespaces/thredds/InvCatalog/v1.0");
    private static final Namespace wms = Namespace.getNamespace("http://www.opengis.net/wms");
    ...
    private static final Namespace gmi = Namespace.getNamespace("gmi", "http://www.isotc211.org/2005/gmi");
    private static final Namespace gmx = Namespace.getNamespace("gmx", "http://www.isotc211.org/2005/gmx");
    private static final Namespace gsr = Namespace.getNamespace("gsr", "http://www.isotc211.org/2005/gsr");
    private static final Namespace gss = Namespace.getNamespace("gss", "http://www.isotc211.org/2005/gss");
    private static final Namespace gts = Namespace.getNamespace("gts", "http://www.isotc211.org/2005/gts");
    private static final Namespace gml32 = Namespace.getNamespace("gml", "http://www.opengis.net/gml/3.2");
    ...
    private Map<String, ThreddsMetadata.Variable> gridVariables = new HashMap<>();
    private boolean metadataObtained = false;
    private boolean datasetMetadataObtained = false;
    private String metadataGetService;
    private List<Namespace> iso191152NamespaceList = new ArrayList<>();
    ...
    public HarvestResult harvest(Logger log) throws Exception {
        ...
        for (String localUri : localUris.getUris()) {
            ...
            if (!harvestUris.contains(localUri)) {
                for (RecordInfo recordInfo : localUris.getRecords(localUri)) {
                    if (cancelMonitor.get()) {
                        return this.result;
                    }

                    if (log.isDebugEnabled())
                        log.debug("  - Removing deleted metadata with id: " + recordInfo.id);
                    mdManager.deleteMetadata(context, recordInfo.id);

                    if (recordInfo.isTemplate.equals("s")) {
                        //--- Uncache xlinks if a subtemplate
                        Processor.uncacheXLinkUri(metadataGetService + recordInfo.uuid);
    ...
    private void harvestCatalog(Element cata) throws Exception {
        ...
        catalog = factory.readXML(params.url);
    ...
    private boolean hasThreddsMetadata(InvDataset ds) {
        ...
        List<InvDocumentation> documentationList = ds.getDocumentation();
        log.debug("ThreddsMetadata: " + gsC + " : " + dr + " : " + vars.size() + " : " + documentationList);
        return (gsC != null || dr != null || !vars.isEmpty() || documentationList != null);
    }
    ...
    private void saveMetadata(Element md, String uuid, String uri, boolean isService) throws Exception {
        ...
        if (!isService) {
            if (params.datasetCategory != null && !params.datasetCategory.equals("")) {
                ...
                metadata.getMetadataCategories().add(metadataCategoryOptional.get());
            }
        } else {
    ...
    private void examineThreddsDataset(InvDataset ds) {
        getMetadata(ds);
    ...
    private String getServerOperations(InvService serv, InvDataset ds) {
        String serverOperations = "none";
        if (serv.getServiceType() == ServiceType.OPENDAP) {
            InvAccess access = ds.getAccess(ServiceType.OPENDAP);
            if (access != null) {
                String href = access.getStandardUrlName() + ".help";
                String readResult = getResultFromHttpUrl(href);
                if (readResult != null) serverOperations = readResult;
            }
        }
    ...
    private String getResultFromHttpUrl(String href) {
        ...
        try {
            ...
            try {
                ...
                resultFromHttpUrl = version.toString();
    ...
    private void deleteExistingMetadata(String uri) throws Exception {
        ...
        for (RecordInfo recordInfo : localRecords) {
            mdManager.deleteMetadata(context, recordInfo.id);
    ...
    private Element getXMLResponse(String url) throws IOException {
        XmlRequest req = context.getBean(GeonetHttpRequestFactory.class).createXmlRequest();
        req.setUrl(new URL(url));
        req.setMethod(AbstractHttpRequest.Method.GET);
        Lib.net.setupProxy(context, req);
...
// ISO Stuff
    ...
    private boolean extractISOVariables(List<?> bands) {
        boolean extractedVariables = false;
                        ...
                        if (layerName.length() > 0) {
                            ...
                            if (desc != null) {
                                ...
                                if (layerDesc.length() > 0) {
                                    variable.setDescription(layerDesc);
                                }
                            }
                            gridVariables.put(layerName, variable);
                        ...
                        for (Object oa : attrs) {
                            if (oa instanceof Element) {
                                ...
                                if (attrName.equals("long_name")) {
                                    variable.setDescription(attr.getAttributeValue("value"));
                                } else if (attrName.equals("units")) {
                                    variable.setUnits(attr.getAttributeValue("value"));
                                }
                            }
                        }
                        gridVariables.put(name, variable);
                        extractedVariables = true;
                    }
                }
            }
                ...
                if (name != null) {
                    ...
                    for (Object oa : attributes) {
                        if (oa instanceof Element) {
                            ...
                            String attrName = attr.getAttributeValue("name");
                            if (attrName.equals("long_name")) {
                                variable.setDescription(attr.getChildText("value"));
                            } else if (attrName.equals("units")) {
                                variable.setUnits(attr.getChildText("value"));
                            }
                        }
                    }
                    gridVariables.put(name, variable);
...
// End DDX Stuff
    ...
    private Element turnThreddsMetadataVariablesIntoXml() {
        Element resultEl = new Element("gridDataset");
        Element gridSet = new Element("gridSet");
        for (ThreddsMetadata.Variable variable : gridVariables.values()) {
            Element grid = new Element("grid");
            grid.setAttribute("name", variable.getName());
            Element attr = new Element("attribute");
            attr.setAttribute("name", "long_name");
            attr.setAttribute("value", variable.getDescription());
            grid.addContent(attr);
            attr = new Element("attribute");
            attr.setAttribute("name", "units");
            attr.setAttribute("value", variable.getUnits());
            grid.addContent(attr);
            gridSet.addContent(grid);
        }
        resultEl.addContent(gridSet);
        log.debug("Thredds variables have been turned into: " + Xml.getString(resultEl));
        return resultEl;
    }
    ...
    private Element addDataParameters(Element md, Element dataParameters) {
