...
/** Read elements matching a simple path expression from a document stream
 * Doesn't load the whole document into memory (uses StAX to read the elements 
 * returning them as JDOM elements).  For use processing large documents
 * 
  */
...
public class XmlElementReader implements Iterator<Element> {
	...
	//---------------------------------------------------------------------------
    /**
     * Class encapsulating matching elements against a simple path expression
	...
	private static class WildcardTest extends NameTest {
		...
		public boolean matches(QName name) {return true;}
	}
	...
	private static class EqualityTest extends NameTest {
		...
		public boolean matches(QName name) {return this.name.equals(name);}
	}
