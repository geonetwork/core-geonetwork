...
<xsl:stylesheet version="2.0"
  ...
  <xsl:variable name="javadocBaseURL" select="'http://geonetwork-opensource.org/manuals/trunk/eng/developer/apidocs/geonetwork/'"/>

  <xsl:variable name="githubBaseURL" select="'https://github.com/geonetwork/core-geonetwork/blob/develop/web/src/main/webapp/'"/>
...
@ngdoc overview
@name  <xsl:value-of select="substring-after(substring-before($fileName, '.xml'), 'config-')"/>
@description
...
Configuration file [<xsl:value-of select="substring-before($fileName, '.xml')"/>](<xsl:value-of select="concat($githubBaseURL, 'WEB-INF/config/', $fileName)"/>)
...
## <xsl:text>&lt;a id="</xsl:text><xsl:value-of select="@name"/><xsl:text>"&gt;&lt;/a&gt;</xsl:text> Service <xsl:value-of select="@name"/>
...
<xsl:text>
    ...
    <xsl:for-each select="class">
      ...
      <xsl:variable name="javadocURL" select="concat($javadocBaseURL, translate($class, '.', '/'), '.html')"/>
...
* Output XSLT: [<xsl:value-of select="substring-after(output/@sheet, '../')"/>](<xsl:value-of select="$xslURL"/>)

      <xsl:if test="output/xml">
        <xsl:variable name="locURL" select="concat($githubBaseURL, 'loc/eng/', output/xml/@file)"/>
* i18n resource: [<xsl:value-of select="output/xml/@name"/>](<xsl:value-of select="$locURL"/>)
      </xsl:if>
    </xsl:if>
...
* Security: ** <xsl:value-of select="$configSecurity//sec:intercept-url[contains(@pattern, concat('/', $serviceName))]/@access"/>**
