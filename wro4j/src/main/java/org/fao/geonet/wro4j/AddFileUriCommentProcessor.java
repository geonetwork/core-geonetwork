...
/**
 * This processor is only to be enabled when the maven env variable is in dev mode because it adds debug information to the css and
 * javascript files that assist in debugging the css and javascript.  See dev.properties, prod.properties and inspire.properties and
 * wro.properties.
 * <p/>
 * This class adds a comment to js and css files with the name of the resource so that it is possible to identify in the grouped
 * "views" of wro4j which file contains the code to change.  In the case of js it adds a comment of the form
 ...
 * <p/>
 * In the case of css it is a little trickier because of 2 factors.
 * <ul>
 * <li>imports - css imports are resolved before minification and as such the minification would remove the comment</li>
 * <li>less - the less compiler removes comments as well and again the comment cannot be used</li>
 * </ul>
 * <p></p>
 * The strategy used for css and less us as follows
 * <ol>
 * <li>Add:
 ...
 * </li>
 * <li>Search file for all #start_{xyz} {text-align:center} parts and whitespace is added around the block.
 * This is done because they are added by the import tags and the minified code would have removed all whitespace around the css block
 * White space is added to make it easier to read the minified code.
 * <p/>
 * </li>
 * <p/>
 * </ol>
