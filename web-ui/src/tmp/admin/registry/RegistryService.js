...
(function () {
  goog.provide("sx_registry_service");
    ...
    function ($http, $q) {
      ...
      this.guessTool = function (url) {
        ...
        $http({
          ...
          function (r) {
            deferred.resolve(r.data["@graph"] ? "ldRegistry" : "re3gistry");
          },
          function (r) {
            ...
            if (type === "ldRegistry" && r.data["@graph"]) {
              angular.forEach(r.data["@graph"], function (value, key) {
                var labels = value["rdfs:label"],
                  ...
                  label = angular.isArray(labels)
                    ? labels[0]["@value"]
                    : angular.isObject(labels)
                    ? labels["@value"]
                    : labels,
                  description = angular.isArray(descriptions)
                    ? descriptions[0]["@value"]
                    : angular.isObject(descriptions)
                    ? descriptions["@value"]
                    : descriptions;
                if (label !== "root") {
                  itemClass.push({
      ...
      this.loadItem = function (url, collection, lang) {
        return $http({
          url: url + "/" + collection + "/" + collection + "." + lang + responseFormat,
          method: "GET",
          cache: true
        });
      };
