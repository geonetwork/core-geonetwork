...
(function () {
  goog.provide("sx_onlinesrc_service");
      ...
      return {
        ...
        formatResources: function (data, lang, mdLanguage) {
          ...
          angular.forEach(data.onlines, function (src) {
            ...
            var hasAnnotations =
              src.protocol === "OGC:WMS" &&
              data.onlines.some(function (otherSrc) {
                return (
                  otherSrc.protocol === "ANNOTATIONS" && otherSrc.lUrl.endsWith(name)
                );
              });
            if (hasAnnotations) {
        ...
        removeFeatureCatalog: function (onlinesrc) {
          var params = {
            uuid: gnCurrentEdit.uuid,
            uuidref: onlinesrc["@subtype"] ? onlinesrc.url : onlinesrc.id
          };
