...
(function () {
  goog.provide("sx_onlinesrc_directive");

  goog.require("ga_print_directive");
  goog.require("sx_utility");
  goog.require("sx_filestore");
        ...
        return {
          ...
          link: function (scope, element, attrs) {
            ...
            var loadRelations = function () {
              gnOnlinesrc.getAllResources(["thumbnail"]).then(function (data) {
          ...
          link: {
            ...
            post: function (scope, element, attrs) {
              ...
              scope.addOnlinesrc = function () {
                ...
                // SEXTANT SPECIFIC
                if (
                  scope.params.protocol === "OGC:WMS" &&
                  scope.params.annotationsEnabled
                ) {
                  var annotationsApiUrl = new URL(
                    window.location.origin +
                      window.location.pathname +
                      "../../../api/annotations/"
                  );
                  var layerName =
                    $filter("gnLocalized")(scope.params.name) || scope.params.name;
                  processParams.annotationsUrl =
                    annotationsApiUrl + scope.gnCurrentEdit.uuid + "_" + layerName;
                }
                // END SEXTANT SPECIFIC
