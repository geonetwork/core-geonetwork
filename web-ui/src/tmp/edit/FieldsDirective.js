...
(function () {
  goog.provide("sx_fields_directive");

  goog.require("sx_metadata_manager_service");
  ...
  module.directive("gnCheckpointCptCovered", function () {
    return {
      restrict: "A",
      templateUrl:
      ...
      link: function (scope, element, attrs) {
        scope.options = [
          ...
          {
            label: "Component is not covered because existing data not available",
            status: false
          },
        ...
        function update() {
          $("#" + scope.id).toggleClass("hidden", scope.covered.status);
          $("#" + scope.id + "-table").toggleClass("hidden", !scope.covered.status);
      ...
      link: function (scope, element, attrs) {
        ...
        function init() {
          ...
          if (tokens == null) {
            ...
            scope.isMandatory = tokens.length === 3 ? tokens[2] === "mandatory" : true;
            scope.lineage = tokens.length === 3 ? tokens[1].trim() : field.value.trim();
          }
        }
        ...
        function buildValue() {
          if (scope.lineage == "") {
            ...
            return (
              scope.lineage + " (" + (scope.isMandatory ? "mandatory" : "optional") + ")"
            );
          }
        }
  ...
  module.directive("gnTitle", [
    ...
    function ($http, $rootScope, $translate, gnCurrentEdit, gnSearchManagerService) {
      return {
        restrict: "C",
        link: function (scope, element, attrs) {
          ...
          var titleCheck = function () {
            ...
            // edited.
            $http
              .get("q?fast=index&_content_type=json&_title=" + titleEl.val(), {
                cache: true
              })
              .then(function (r) {
                ...
                if (records.metadata) {
                  ...
                  for (var i = 0; i < records.metadata.length; i++) {
                    if (records.metadata[i].id !== gnCurrentEdit.id) {
                      count++;
                    }
                  }
