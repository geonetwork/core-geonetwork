  ...
  module.controller('GnNewMetadataController', [
    ...
    function($scope, $routeParams, $http, $rootScope, $translate, $compile,
      ...
      $scope.createNewMetadata = function(isPublic) {
        ...
        if (!$scope.generateUuid && ($scope.urnSelectedTemplateId != 1)) {

          // Custom URN
          if ($scope.urnSelectedTemplateId == 0) {
            metadataUuid = $scope.urnCustom;

          // Template URN
          } else {
            metadataUuid = getSelectedUrnTemplate().template;
      ...
      $scope.updateURNTemplate = function() {
        if ($scope.urnSelectedTemplateId <= 1) return;

        var selectedTemplate = getSelectedUrnTemplate();

        $scope.urnSelectedTemplateForLabel = selectedTemplate.template
      ...
      $scope.updateURNTemplateLabel = function() {
        $scope.urnSelectedTemplateForLabel = getSelectedUrnTemplate().template;
      ...
      $scope.showCustomURNField = function() {
        if (!$scope.urnSelectedTemplateId) return false;

        return ($scope.urnSelectedTemplateId == 0);
      };
      ...
      $scope.isURNFilled = function() {
        if ($scope.urnSelectedTemplateId == 1) return true;
        if ($scope.urnSelectedTemplateId == 0) return $scope.urnCustom;
      ...
      function getSelectedUrnTemplate(id) {
        var selectedTemplate;

        for (var i = 0; i < $scope.urnTemplates.length; i ++) {
          if ($scope.urnTemplates[i].id == $scope.urnSelectedTemplateId) {
            selectedTemplate = $scope.urnTemplates[i];
            break;
          }
        }
      ...
      function loadUrnTemplates() {
        ...
        $http.get('urntemplate?_content_type=json') // 'info?type=urnTemplates&_content_type=json&all=true')
