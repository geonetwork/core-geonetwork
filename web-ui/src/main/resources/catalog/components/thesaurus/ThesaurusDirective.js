...
(function() {
  ...
  module.directive('gnKeywordPicker', [
    'gnThesaurusService', '$compile', '$translate', '$timeout', 'gnCurrentEdit',
    function(gnThesaurusService, $compile, $translate, $timeout, gnCurrentEdit) {
      return {
        ...
        link: function(scope, element, attrs) {
          ...
          scope.templateFieldElementWithConceptId = attrs.templateFieldElementWithConceptId || '';
          scope.xmlSnippet = '';
          ...
          var buildTemplateXmlSnippet = function() {
            ...
            if(scope.conceptIdElementName) {
              elementXmlSnippet =  "<" + scope.templateFieldElementWithConceptId + " xlink:href='" +
                scope.templateFieldConceptIdValue + "'>" +
                scope.templateFieldValue + "</" + scope.templateFieldElementWithConceptId + ">";
            } else {
              elementXmlSnippet = "<" + scope.templateFieldElement + ">" +
                scope.templateFieldValue + "</" + scope.templateFieldElement + ">";
            }
          ...
          var init = function() {
            ...
            if (angular.isDefined(attrs.thesaurusConceptIdAttribute)) {
              if (scope.templateField) {
                scope.conceptIdElementName =
                  ...
                  (isMultilingualMode ? '_' + attrs.id.replace('gn-field-', '') : scope.templateFieldId) +
                ...
                input = element.parent().parent().find('#' + scope.conceptIdElementName);
              } else {
                scope.conceptIdElementName =
                  // In multilingual mode, the ref to the CharacterString is known using the id
                ...
                // Check that the element does not exist already in the form
                // Could be in the case it was already encoded.
                input = element.parent().parent().find('[name=' + scope.conceptIdElementName + ']');
              }
              ...
              if ((!isMultilingualMode && input.length === 0) ||
                ...
                var conceptIdElement =  angular.element(
                  ...
                  '    <div class="col-sm-6"><input ' + inputPropertyName + '="' + scope.conceptIdElementName + '" ' +
            ...
            if (scope.templateField) {
              ...
              var inputTemplateField = element.parent().parent().find('#' + scope.templateFieldId);

              if (inputTemplateField.length == 0) {
                var inputTemplateFieldHtml =
                  '<input type="hidden" id="' + scope.templateFieldId + '" value="{{xmlSnippet}}" />';
              ...
              $.proxy(function(obj, keyword) {
                ...
                if(scope.conceptIdElementName) {
                  ...
                  if (scope.templateField) {
                    ...
                    input = insertionPoint.find('[name=' + scope.conceptIdElementName + ']');
                  }
          ...
          if (scope.templateField) {
            scope.$watch('xmlSnippet', function () {
              if (scope.templateFieldId) {
                ...
                $timeout(function () {
                  $("#" + scope.templateFieldId).val(scope.xmlSnippet).change();
                });
              }
            });
          }
