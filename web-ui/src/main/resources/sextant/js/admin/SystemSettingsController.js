  ...
  module.controller("GnSystemSettingsController", [
    ...
    function (
            ...
            .put(
              "../api/ui" + (isUpdate ? "/" + newid : ""),
              {
                ...
                configuration: isUpdate
                  ...
                  : JSON.stringify(gnGlobalSettings.gnCfg)
              },
              { responseType: "text" }
            )
            .then(
      ...
      $scope.processRecommended = function (processName) {
        ...
        $scope.processTitle = $translate.instant("processRecommendedOnHostChange-help", {
      ...
      $scope.processRecommendedForId = function (processName) {
        ...
        $scope.processResourceTitle = $translate.instant(
          "processRecommendedOnHostChange-help",
          {
            old: buildUrl($scope.initalSettings),
            by: buildUrl($scope.settings)
          }
      ...
      $scope.saveAndProcessSettings = function (formId, process) {
        ...
        $location
          .path("/tools/batch/select/all/process/" + process)
          .search(
            "urlPrefix=" +
              buildUrl($scope.initalSettings) +
              "&newUrlPrefix=" +
              buildUrl($scope.settings)
          );
      };
      ...
      $scope.executeAtomHarvester = function () {
        return $http
          .get("atomharvester?_content_type=json")
          .success(function (data) {
