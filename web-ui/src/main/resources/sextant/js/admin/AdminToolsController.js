  ...
  module.controller("GnAdminToolsController", [
    ...
    function (
      ...
      function checkLastBatchProcessReport() {
        // Check if processing
        return $http.get("../api/processes/reports").success(function (data, status) {
          ...
          if ($scope.processReport && $scope.processReport.running) {
      ...
      $scope.removeReplacement = function (replacement) {
        $scope.replacer.replacements.splice(
          $scope.replacer.replacements.indexOf(replacement),
          1
        );
      };
      ...
      $scope.downloadReplacementConfig = function ($event) {
        var content =
          ...
          encodeURIComponent(JSON.stringify($scope.replacer.replacements));
        $($event.target)
          .parent("a")
      ...
      $scope.$watch("replacer.group", function (newValue, oldValue) {
        ...
        $scope.replacer.elements = $scope.selectedProcess.config[newValue].elements;
      });
