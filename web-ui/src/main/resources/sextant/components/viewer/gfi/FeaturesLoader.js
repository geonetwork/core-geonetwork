...
(function () {
  ...
  geonetwork.GnFeaturesGFILoader.prototype.loadAll = function () {
    ...
    // Sextant specific
    var infoFormat = this.ncwmsService.isLayerNcwms(layer)
      ? "text/xml"
      ...
      if ($.inArray(infoFormat, layer.get("capRequest").GetFeatureInfo.Format) == -1) {
        ...
        var friendlyFormats = layer
          ...
          .GetFeatureInfo.Format.filter(function (el) {
            return (
              el.toLowerCase().localeCompare("application/json") == 0 ||
    ...
    this.promise = this.$http
      ...
      .then(
        function (response) {
          // Sextant specific
          if (
            this.ncwmsService.isLayerNcwms(layer) ||
            this.ncwmsService.isLayerOceanotron(layer)
          ) {
            ...
            this.features =
              props.value && props.value != "none" ? [new ol.Feature(props)] : [];
          } else if (
            infoFormat &&
    ...
    if (uuid) {
      this.dictionary = this.$http
        .get("../api/records/" + uuid + "/featureCatalog?_content_type=json")
        .then(
          function (response) {
  ...
  geonetwork.GnFeaturesINDEXLoader.prototype.formatUrlValues_ = function (url) {
    ...
    return (
      '<a target="_blank" href="' + encodeURI(processedUrl) + '">' + linkTpl + "</a>"
    );
  };
  ...
  geonetwork.GnFeaturesINDEXLoader.prototype.fillUrlWithFilter_ = function (url) {
    ...
    while (match && match.length === 4) {
      try {
        ...
        var idxName = parseValue
          ? filterName
          : this.indexObject.getIdxNameObj_(filterName).idxName;
        var fValue = indexFilters.qParams && indexFilters.qParams[idxName];

        if (fValue !== null && fValue !== undefined) {
          fValue = parseValue
            ? fValue.values[filter.split(".")[1]]
        ...
        console.warn(
          "[sxt] Error while replacing parameters in features URL attribute, " +
            "param: " +
            urlParam +
            ", error: ",
          e
        );
        url = url.replace(placeholder, ""); // Remove placeholder to avoid infinite loop.
      } finally {
