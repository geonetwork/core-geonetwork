...
(function () {
  goog.provide("sx_ncwms_service");

  var module = angular.module("gn_ncwms_service", []);

  var OCEANOTRON_INFO_URL_TEMPLATE = "**path**/OPENDAP/opendap/**layer**.dds";
  ...
  module.service("gnNcWms", [
    ...
    "gnGlobalSettings",
    "$q",
    function (gnMap, gnUrlUtils, gnOwsCapabilities, $http, gnGlobalSettings, $q) {
      /**
       * @ngdoc method
      ...
      this.feedOlLayer = function (layer) {
        if (layer === undefined) {
          return $q.reject("Undefined layer");
        }
      ...
      this.formatNcwmsAvailableDates = function (layer) {
        ...
        var isRange =
          layer.get("time").values[0] &&
          layer.get("time").values[0].toString().indexOf("/") > -1;
      ...
      this.initOceanotronParams = function (layer) {
        ...
        var promise2 = this.getColorRangesBounds(layer, [-180, -90, 180, 90]).then(
          function (response) {
      ...
      this.parseStyles = function (layer) {
        ...
        if (this.isLayerNcwms(layer)) {
          var metadata = layer.get("advancedMetadata");
          if (
            angular.isArray(metadata.supportedStyles) &&
      ...
      this.updateLegendUrl = function (legendUrl, layerType, params) {
        ...
        var p = parts.length > 1 ? gnUrlUtils.parseKeyValue(parts[1]) : {};
      ...
      this.getOceanotronInfoUrl = function (layer) {
        ...
        var path = u.protocol + "://" + u.host + "/" + u.pathname.split("/")[1];
        return OCEANOTRON_INFO_URL_TEMPLATE.replace("**layer**", name).replace(
          "**path**",
          path
        );
      };
      ...
      this.getOceanotronInfo = function (layer) {
        return $http.get(this.getOceanotronInfoUrl(layer)).then(function (response) {
          var type;
       ...
       */
      this.getResultImageUrl = function (
        layer,
        proj,
        resolution,
        geom,
        ...
        switch (layer.get("advancedType")) {
          ...
          case LAYERTYPE_WMS_OCEANOTRON:
            ...
            gfiUrl = layer
              .getSource()
              .getGetFeatureInfoUrl(geom.getCoordinates(), resolution, proj, layerParams);
            return $http.get(gfiUrl).then(function (response) {
              var ids = me.parseOceanotronXmlCapabilities(response.data);

              if (!ids) return null;
              return me.getNcwmsServiceUrl(layer, proj, geom.getCoordinates(), service, {
                LAYER: ids
              });
            });
        }
      ...
      this.isLayerAdvancedWms = function (layer) {
        return (
          layer.get("advancedType") === LAYERTYPE_WMS ||
