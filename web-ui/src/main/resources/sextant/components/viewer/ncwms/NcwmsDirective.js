...
(function () {
  goog.provide("sx_ncwms_directive");
        ...
        link: function (scope, element, attrs) {
          ...
          var activateInteraction = function (activeTool) {
            ...
            drawInteraction.on(
              "drawend",
              function (evt) {
                var promiseUrl = gnNcWms.getResultImageUrl(
                  scope.layer,
          ...
          var initFormValues = function () {
            ...
            if (scope.isLayerNcwms()) {
              ...
              if (layerMetadata.units) {
                ...
                if (
                  scope.isLayerOceanotron() &&
                  scope.layer.get("oceanotronScaleRange")
                ) {
                ...
                scope.colorscale = {
                  range: [scope.colorRange.min, scope.colorRange.max]
                };
              }
            ...
            if (elevation) {
              ...
              if (scope.isLayerOceanotron()) {
                ...
                scope.elevRange =
                  scope.elevationMin.toFixed(1) +
                  elevation.units +
                  " / " +
                  scope.elevationMax.toFixed(1) +
                  elevation.units;
          ...
          scope.updateLayerParams = function () {
            ...
            scope.layer.set(
              "legend",
              gnNcWms.updateLegendUrl(
                scope.layer.get("legend"),
                scope.layer.get("advancedType"),
                angular.extend(
                  {
