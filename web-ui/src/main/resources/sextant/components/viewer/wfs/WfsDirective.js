...
(function () {
        ...
        link: function (scope, element, attrs, ctrls) {
          ...
          scope.checkWFSUrl = function () {
            if (scope.url && scope.url != "") {
              return gnWfsService
                .getCapabilities(gnGlobalSettings.getNonProxifiedUrl(scope.url))
                .then(
                  function (capabilities) {
                    scope.isInitialized = true;
                    scope.isWfsAvailable = true;
                    scope.capabilities = capabilities;
                    scope.featureType = gnWfsService.getTypeName(
                      capabilities,
                      scope.typename
                    );
          ...
          scope.downloadFormatChange = function (o) {
            ...
            if (df) {
              scope.download(df.split("#")[0], df.split("#")[1] == "true");
            }
          };
  ...
  module.directive("gnNoMapWfsDownload", [
    "gnWfsService",
