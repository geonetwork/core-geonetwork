      ...
      return {
        ...
        link: function (scope, element, attrs) {
            ...
            function (newLink, oldLink) {
                ...
                scope.applicationProfile.inputs.forEach(function (input) {
                  if (input.bindWmsNameToWPS) {
                    ...
                    availableLayers.forEach(function (layerName) {
                      if (input.maxOccurs && count === input.maxOccurs) {
                        return;
                      }
              ...
              scope.reOrderLayerInputs = function (key) {
                ...
                var layersOrderedList = scope
                  ...
                  .map(function (ll) {
                    return {
                      name: ll.get("label") || ll.get("name"),
                      qi: ll.get("qi_list")[key]
                    };
                  })
                  .sort(function (a, b) {
              ...
              if (scope.wfsLink) {
                ...
                if (esObject) {
                  ...
                  Object.keys(currentFilters).forEach(function (key) {
                    var name =
                      key == "geometry" ? key : esObject.getIdxNameObj_(key).name;
                      ...
                      angular.forEach(
                        ...
                        function (input) {
                          ...
                          if (
                            ...
                            scope.applicationProfile.inputs.forEach(function (input) {
                              if (input.identifier == inputName) {
                                ...
                                var delimiter =
                                  isTokenized && (input.wfsFilterValuesDelimiter || ",");
                                ...
                                if (
                                  ...
                                  if (valueIndex >= 0) {
                                    ...
                                    wfsFilterValue = [
                                      wfsFilterValues[wfsFilter].join(delimiter)
                                    ];
                                  } else {
                          ...
                          if (wfsFilterValue && wfsFilterValue.length) {
                            scope.wfsInputValues[inputName] = wfsFilterValue
          ...
          scope.submit = function () {
            ...
            if (attrs["executeCallback"]) {
              var message = gnWpsService.printExecuteMessage(
                scope.processDescription,
                inputs,
                output
              );
          ...
          scope.getAllInputs = function () {
            ...
            return scope.wpsLink.inputs.map(function (input) {
              var innerIndex = scope
                .getInputsByName(input.name)
                .map(function (input) {
                  return input.name;
                })
                .indexOf(input.name);
              return wfsValues[input.name] && wfsValues[input.name].length > innerIndex
                ? wfsValues[input.name][innerIndex]
                : input;
            });
          };
