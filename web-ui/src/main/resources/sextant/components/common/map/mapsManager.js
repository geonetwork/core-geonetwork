        ...
        createMap: function (type) {
          ...
          if (type === this.VIEWER_MAP) {
            ...
            if (gnViewerSettings.owscontext) {
              console.warn(
                'Url parameter usage "catalog.search?owscontext" is deprecated.\nPlease use catalog.search#/map?owscontext= instead.'
              );
            ...
            if (gnViewerSettings.wmsurl && gnViewerSettings.layername) {
              console.warn(
                'Url parameter usage "catalog.search?wmsurl=...&layername=..." is deprecated.\nPlease use catalog.search#/map?wmsurl=...&layername=... instead.'
              );
          ...
          if (!mapReady && config.context) {
            mapReady = gnOwsContextService.loadContextFromUrl(config.context, map);
          }
                  ...
                  gnMap
                    ...
                    .then(function (layer) {
                      ...
                      if (layer.get("cextent") && map.get("addedExtent")) {
                        map.set(
                          "addedExtent",
                          ol.extent.extend(layer.get("cextent"), map.get("addedExtent"))
                        );
                        applyAddedExtent(map);
                      }
                      // end specific sextant
