      ...
      function (
        ...
        var getTheLayerFromMap = function (map, name, url, style) {
          for (var i = 0; i < map.getLayers().getLength(); i++) {
            ...
            if (source instanceof ol.source.WMTS && l.get("url") == url) {
              if (l.get("name") == name) {
                return l;
              }
            } else if (
        ...
        var getImageSourceRatio = function (map, maxWidth) {
          ...
          var width =
            (map.getSize() && map.getSize()[0]) || $(".gn").width() + scrollBarWidth;
              ...
              var vectorSource = new ol.source.Vector({
                ...
                loader: function (extent, resolution, projection) {
                  ...
                  if (getCapLayer.version == "1.1.0") {
                    urlGetFeature = gnUrlUtils.append(
                      parts[0],
                      gnUrlUtils.toKeyValue({
                        service: "WFS",
                        request: "GetFeature",
                      ...
                      $q.resolve(olL)
                        ...
                        .finally(function () {
                          ...
                          if (wfsConfigs && wfsConfigs.length) {
                            ...
                            var esObj = wfsFilterService.getEsObject(
                              wfsConfig.url,
                              wfsConfig.name
                            );
                            if (esObj && esObj.initialFilters) {
                              ...
                              var sldConfig = wfsFilterService.createSLDConfig(
                                esObj.initialFilters.qParams
                              );
                              if (sldConfig.filters.length > 0) {
                                wfsFilterService
                                  .getSldUrl(sldConfig, olL.get("url"), wfsConfig.name)
                                  .then(function (sldURL) {
