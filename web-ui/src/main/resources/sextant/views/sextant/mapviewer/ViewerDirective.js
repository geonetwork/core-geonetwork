(function () {
          ...
          return {
            pre: function preLink(scope, iElement, iAttrs, controller) {
              ...
              $($window).on("resize", function () {
                setTimeout(map.updateSize.bind(map), 100);
              });
              ...
              scope.isContainerOpened = function () {
                try {
                  return (
                    scope.activeTools.layers ||
                    scope.activeTools.import ||
                    scope.activeTools.contexts ||
                    scope.activeTools.print ||
                    scope.mInteraction.active ||
                    scope.drawVector.active ||
                    scope.activeTools.benthique ||
                    scope.activeTools.processes ||
              ...
              scope.mInteraction = gnMeasure.create(map, scope.measureObj, scope);
  ...
  module.directive("sxtMousePosition", [
    function () {
      return {
        restrict: "A",
        templateUrl:
    ...
    function (gnGeometryService, gnWpsService, gnProfileService, $http, $q) {
      return {
        ...
        link: function (scope, element, attrs) {
          ...
          gnWpsService.describeProcess(processUri, processId).then(function (data) {
          ...
          drawInteraction.on("drawend", function (event) {
            ...
            var geometryOutput = gnGeometryService.printGeometryOutput(
              scope.map,
              event.feature,
              {
                format: "gml",
  ...
  module.directive("sxtFullScreen", [
    function () {
      return {
        restrict: "A",
        link: function (scope, element, attrs) {
  ...
  module.directive("sxtPopoverDropdown", [
    "$timeout",
    function ($timeout) {
      return {
        restrict: "A",
        link: function (scope, element, attrs) {
          ...
          var onMousedown = function (e) {
            if (
              ...
              button.data("bs.popover").$tip.find(".dropdown-menu")[0] != e.target &&
              ((!grid &&
      ...
      return {
        ...
        link: function (scope, element, attrs) {
          ...
          container.on("focus", 'input[type="text"]', function (event) {
            ...
            buttons
              .eq($(event.target.closest(".panel-carousel")).index())
              .addClass("active");
          });
