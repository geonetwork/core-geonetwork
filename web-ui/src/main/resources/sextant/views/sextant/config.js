  ...
  module.run([
    ...
    function (
      ...
      if (typeof sxtSettings != "undefined") {
        ...
        var params = gnUrlUtils.parseKeyValue(window.location.search.replace(/^\?/, ""));
        viewerSettings.owscontext =
          params.owscontext && decodeURIComponent(params.owscontext);
        viewerSettings.wmsurl = params.wmsurl;
        viewerSettings.layername = params.layername;
        viewerSettings.layergroup = params.layergroup;
      }

      gnGlobalSettings.isMapViewerEnabled = gnGlobalSettings.isMapViewerEnabled || true;

      /** *************************************
       * Define mapviewer background layers
       */
      viewerSettings.bgLayers = [new ol.layer.Tile()];
      ...
      if (typeof sxtSettings != "undefined") {
        ...
        if (
          angular.isUndefined(searchSettings.tabOverflow.map) ||
        ...
        if (sxtSettings.sortBy) {
          searchSettings.sortbyValues = sxtSettings.sortBy.map(function (criteria) {
            var name = criteria;
            var direction = "";
          ...
          searchSettings.filters = [
            {
              query_string: {
                query:
                  "+resourceType:(" +
                  sxtSettings.metadataType.replaceAll(" or ", " OR ") +
                  ")"
              }
            }
          ];
      ...
      if (searchSettings.configWho) {
        searchSettings.filters.push({
          query_string: {
            query:
              '+OrgForResource:("' +
              searchSettings.configWho.replace(/,/g, '" OR "') +
              '")'
          }
        });
      }
      ...
      if (searchSettings.hideSourceRelations) {
        searchSettings.grid.related = searchSettings.grid.related.filter(function (type) {
      ...
      if (searchSettings.configWhat) {
        searchSettings.filters.push({
          query_string: {
            query:
              '+groupPublished:("' +
              searchSettings.configWhat.replace(/,/g, '" OR "') +
              '")'
          }
        });
        if (esFacetConfig.group) {
          sxtService
            .getGroupIdsFromConfigWhat(searchSettings.configWhat)
        ...
        if (angular.isDefined(sxtSettings.localisations)) {
          ...
          if (angular.isArray(viewerSettings.localisations)) {
            viewerSettings.localisations.forEach(function (loc) {
              loc.extent = ol.proj.transformExtent(
                loc.extent,
                "EPSG:4326",
                viewerMap.getView().getProjection()
              );
            });
          }
        }
