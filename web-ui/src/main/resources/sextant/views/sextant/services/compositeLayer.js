      ...
      return {
        init: function (
          ...
          function showHeatmapTooltip(feature) {
            ...
            var topleft = ol.extent.getTopLeft(feature.getGeometry().getExtent());
            heatmapOverlay.setPosition([center[0], topleft[1]]);
            heatmapOverlay.getElement().innerText =
              $translate.instant("featureCount") + ": " + feature.get("count");
          }
          ...
          function showFeatureTooltip(feature, sticky, coordinate) {
            ...
            if (!clickMode && tooltipHoverTemplate) {
              html = defaultHoverTemplate.replace(
                TOOLTIP_ATTR_SUMMARY_REGEX,
                tooltipHoverTemplate
              );
            ...
            while (!!(matches = TOOLTIP_ATTR_SUMMARY_REGEX.exec(html))) {
              token = matches[0];
              attributesSummaryHtml =
                attributesSummaryHtml || me.getFeatureAttributesHtml(feature);
          ...
          function refresh() {
            me.requestCount(featureType, map, layer).then(function (count) {
              if (heatmapMinCount !== undefined && count > heatmapMinCount) {
                layer.setVisible(false);
                gnHeatmapService
                  .requestHeatmapData(featureType, map, me.getQueryParamsFromLayer(layer))
                  .then(function (cells) {
              ...
              if (count <= tooltipMaxCount) {
                ...
                me.requestFeatures(featureType, map, layer, count).then(function (
                  features
                ) {
                  tooltipSource.clear();
        ...
        requestCount: function (featureType, map, layer) {
          var reqParams = this.getQueryObject(
            featureType,
            map,
            layer,
            this.getQueryParamsFromLayer(layer)
          );
          // trigger search on ES
          var url = indexObject.ES_URL.replace("_search", "_count");
          return $http.post(url, reqParams).then(function (response) {
