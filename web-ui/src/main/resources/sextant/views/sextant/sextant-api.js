(function () {
  ...
  var catalogId = scriptEl.hasAttribute("portal")
    ? scriptEl.getAttribute("portal")
    : null;
  var sxtSize = scriptEl.hasAttribute("size") ? scriptEl.getAttribute("size") : null;
  var sxtSizeDiff = scriptEl.hasAttribute("size-diff")
    ? scriptEl.getAttribute("size-diff")
    : null;
  var sizeClass = scriptEl.hasAttribute("max-sm")
    ? "sxt-max-sm"
    : scriptEl.hasAttribute("max-md")
    ? "sxt-max-md"
    : "";
    ...
    .then(function (portalSetting) {
      ...
      Promise.all([
        ...
        function (settings) {
          ...
          if (sxtSizeDiff !== null)
            templateInclude.setAttribute("sxt-size-diff", sxtSizeDiff);
