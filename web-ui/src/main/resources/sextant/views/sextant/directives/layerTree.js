(function () {
  ...
  var module = angular.module("sxt_layertree", ["sxt_compositeLayer"]);
      ...
      return {
        ...
        link: function (scope, element, attrs) {
          ...
          var sortNodeFn = function (a, b) {
            ...
            else if (a.get && a.get("owc_position") !== undefined)
              aPos = a.get("owc_position");

            var bPos = undefined;
            if (b.groupPosition !== undefined) bPos = b.groupPosition;
            else if (b.get && b.get("owc_position") !== undefined)
              bPos = b.get("owc_position");
            ...
            if (group) {
              ...
              if (!newNode) {
                ...
                newNode = {
                  ...
                  path: groupPath,
                  groupPosition:
                    groupPosition && parseInt(groupPosition.split("/")[index])
                };
          ...
          var buildTree = function () {
            ...
            if (
              ...
              sxtSettings.loadContextFolded &&
              scope.layersFromContext
            ) {
          ...
          var filterFn = function (layer) {
            var labelLc = layer.get("label") && layer.get("label").toLowerCase();
            var groupLc = layer.get("group") && layer.get("group").toLowerCase();
            var filterLc = scope.layerFilter.q.toLowerCase();
            return (
              (labelLc && labelLc.indexOf(filterLc)) >= 0 ||
      ...
      return {
        ...
        link: function (scope, element, attrs, controller) {
          ...
          scope.toggleChildrenNodes = function (event, e, forceVisible) {
            if (event && $(event.currentTarget).hasClass("inactiveLayerNode")) {
              return;
            }
            var visible =
              forceVisible !== undefined ? forceVisible : !scope.hasCheckedChildren(e);
          ...
          if (!scope.isParentNode()) {
            ...
            if (scope.member.get("md")) {
              var downloadable = scope.member.get("md").download == true;
              ...
              if (wfsLink) {
                var appProfile;
                try {
                  appProfile = wfsLink.applicationProfile
                    ? JSON.parse(wfsLink.applicationProfile)
                    : {};
                } catch (e) {
                  appProfile = {};
                  console.warn(
                    "Erreur lors de la lecture de l'élément applicationProfile",
                    e
                  );
                }

                if (
                  appProfile.compositeLayer &&
                  !scope.member.get("compositeInitialized")
                ) {
                  ...
                  var tooltipHoverTemplate =
                    appProfile.compositeLayer.tooltipHoverTemplate;
                  if (!tooltipTemplateUrl) {
                    sxtCompositeLayer.init(
                      scope.member,
                      scope.map,
                      featureType,
                      minHeatmapCount,
                      maxTooltipCount,
                      undefined,
                      tooltipHoverTemplate
                    );
                  } else {
                    $http.get(tooltipTemplateUrl).then(
                      function (response) {
          ...
          scope.isOutOfRange = function () {
            if (scope.isParentNode() || !scope.member instanceof ol.layer.Base) {
              return false;
            }
            var mapRes = scope.map.getView().getResolution();
            return (
              scope.member.getMaxResolution() < mapRes ||
