(function () {
  ...
  module.controller("gnsSextant", [
    ...
    function (
      ...
      var unregisterMapsize = $scope.$on("locationBackToSearch", function () {
        if (
          angular.isUndefined(searchMap.getSize()) ||
      ...
      gnViewerSettings.getPreAddLayerPromise = gnNcWms.feedOlLayer.bind(gnNcWms);

      // Manage sextantTheme thesaurus translation
      sxtGlobals.keywords["sextantThemePromise"] = gnThesaurusService
        .getKeywords(
          undefined,
          "local.theme.sextant-theme",
          gnGlobalSettings.locale.iso3lang,
          200
        )
        .then(function (data) {
      ...
      $scope.resultviewFns = {
        addMdLayerToMap: function (link, md, $event) {
          ...
          if (link.protocol.indexOf("OGC:OWS-C") > -1) {
            gnOwsContextService.loadContextFromUrl(link.url, viewerMap);
          ...
          loadLayerPromise.then(function (layer) {
            if (layer) {
              ...
              if (layer.get("cextent") && viewerMap.get("addedExtent")) {
                viewerMap.set(
                  "addedExtent",
                  ol.extent.extend(layer.get("cextent"), viewerMap.get("addedExtent"))
                );
              }
        ...
        addMdLayerToPanier: function (link, md, $event) {
          ...
          if (
            link.protocol.match(
              "WWW:FTP|WWW:DOWNLOAD-1.0-link--download|WWW:DOWNLOAD-1.0-http--download|WWW:OPENDAP|MYO:MOTU-SUB"
            ) != null
          ) {
            if (link.protocol == "MYO:MOTU-SUB") {
              link.url = link.url.replace(
                "action=describeproduct",
                "action=productdownloadhome"
              );
            }
          ...
          for (var i = 0; i < $scope.searchObj.panier.length; i++) {
            if (
              $scope.searchObj.panier[i].link.name === link.name &&
        ...
        addAllMdLayersToPanier: function (layers, md) {
          ...
          if (layersToForm.length) {
            var urls = layersToForm.map(function (layer) {
              return layer.url;
            });
      ...
      $scope.getMapLayersCount = function (layers) {
        return layers.filter(function (layer) {
          return layer.contentType !== "OGC:OWS-C";
        }).length;
      };
      ...
      $scope.$watch("mainTabs.panier.active", function (a) {
        if (a === true) {
          sxtPanierService.bindPanierWithLayers($scope.searchObj.panier, viewerMap);
      ...
      $scope.allowLogin =
        typeof sxtSettings !== "undefined" ? sxtSettings.allowLogin : true;
  ...
  module.controller("gnsSextantSearchForm", [
    ...
    function ($scope, searchSettings) {
      ...
      $scope.$watch("searchObj.params", function (newFilters, oldVal) {
        var currentFilters = [],
          EXCLUDED_PARAMS = [
            "bboxes",
            "bucket",
        ...
        return (
          ...
          searchString
            ...
            .map(function (part) {
              ...
              return part.match(/[a-f0-9]{8}(?:-[a-f0-9]{4}){4}[a-f0-9]{8}/)
                ? part
                : part + "*";
            })
            .join(" ")
