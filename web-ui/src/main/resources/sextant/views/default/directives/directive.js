...
(function () {
  goog.provide("sx_search_default_directive");
  ...
  module.directive("gnDataQualityMeasureRenderer", [
    "$http",
    "$q",
    function ($http, $q) {
      return {
        ...
        link: function linkFn(scope, element, attrs) {
          ...
          function getDpsOrTdpValues(cptId, mId, qm) {
            ...
            if (scope.isUd || scope.isTdp) {
              $http
                ...
                .then(function (r) {
                  if (
                    r.data.hits.hits[0]._source &&
                    r.data.hits.hits[0]._source.dqValues
                  ) {
                    var values = r.data.hits.hits[0]._source.dqValues;
                    for (var i = 0; i < values.length; i++) {
                      var v = values[i];
                      if (
                        v.indexOf(tokens[0] + "/" + tokens[1]) === 0 &&
                        v.indexOf(mId) !== -1
                      ) {
                        var t = v.split("|");
                    ...
                    .then(function (r) {
                      if (
                        ...
                        for (var i = 0; i < values.length; i++) {
                          var v = values[i];
                          if (
                            v.indexOf(tokens[0] + "/" + tokens[1] + "/" + tokens[2]) ===
                              0 &&
                            v.indexOf(mId) !== -1
                          ) {
                            var t = v.split("|");
          ...
          function loadValues() {
            ...
            $http
              ...
              .then(function (r) {
                scope.isUd =
                  r.data.hits.hits[0]._source.standardNameObject.default.indexOf(
                    "Upstream Data"
                  ) !== -1;
                scope.isTdp =
                  r.data.hits.hits[0]._source.standardNameObject.default.indexOf(
                  ...
                  if (cptId.indexOf(scope.cptId) === 0) {
                    if (!isQeOrFu) {
                      ...
                      getDpsOrTdpValues(cptId, value[2], {
                        ...
                        qmStatement: value[8],
                        qm: isNumeric(value[5])
                          ? Math.round(value[5] * 100) / 100 + " " + value[6]
                          : value[5],
                        qeName: qe.name,
                        qe: qe.value,
                        fuName: fu.name,
                        ...
                        scope.components[cptId].measures.push(
                          angular.isArray(values) ? values[0] : values
                        );
                      });
                    }
                  }
