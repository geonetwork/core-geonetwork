 ...
 * 'hits' - outputFormat              (0..1) Can be only 'application/xml' - namespace (0..1) Used
 * for the GET request - outputSchema              (0..1) Can be 'ogccore', 'profile'. Default is
 * 'ogccore' - startPosition             (0..1) Default is 1 - maxRecords (0..1) Default is 10 -
 * TypeNames                 (1..1) A set of 'dataset', 'datasetcollection', 'service',
 * 'application' - elementSetName            (0..1) Can be 'brief', 'summary', 'full'. Default is
 * 'summary' - constraintLanguage        (1..1) Can be 'CQL_TEXT', 'FILTER'. Must be included when
 * 'constraint' is specified - constraintLanguageVersion (1..1) Example '1.0.0' - constraint
 *        (0..1) Query to execute - distributedSearch         (0..1) TRUE|FALSE - hopCount
 * (0..1) default is 2
 */
