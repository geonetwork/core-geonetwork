...
Cancellation of a running task is cooperative. After requesting cancellation, continue polling until the task reaches `CANCELLED`. In a multi-node deployment, the node executing the upload detects a cancellation made through another node during its next task-state synchronization.
...
The filename claim is stored in the database and is therefore enforced across application nodes. A task can initially be accepted and later become `FAILED` if its resolved filename conflicts with another active upload.

Synchronous URL uploads also check for an active asynchronous filename claim before storing the attachment.

### Multi-node operation

Upload-task state and filename claims are stored in the shared GeoNetwork database. Polling and cancellation requests may therefore be handled by any application node.

The download itself, including its active network stream and worker future, remains local to the node that accepted the task. All nodes should use:

- the same GeoNetwork database;
- the same shared attachment-storage configuration; and
- compatible upload-task configuration values.

If the node executing an upload becomes unavailable, another node eventually marks the task as `FAILED` after its heartbeat becomes stale. The upload is not automatically resumed on another node.
...
| -------- | ---- | ----------- |
| `api.params.uploadTaskCorePoolSize` | workers | Number of upload workers normally retained by each application node. |
| `api.params.uploadTaskMaxPoolSize` | workers | Maximum number of upload workers on each node. |
| `api.params.uploadTaskQueueCapacity` | tasks | Number of tasks that may wait for a worker on each node. |
...
| `api.params.uploadTaskUpdateInterval` | milliseconds | Interval between persisted progress/heartbeat updates and cross-node cancellation checks. |
...
With the default worker configuration, each node can execute up to eight uploads concurrently and queue up to 50 additional tasks. Further submissions are rejected and recorded as failed tasks.

Terminal-task and stale-task cleanup runs periodically, so a task may remain stored slightly longer than the configured stale or retention threshold.
