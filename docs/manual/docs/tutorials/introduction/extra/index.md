...
Kibana is an optional component which can be installed with Elasticsearch. Kibana offers a dynamic way to create visualisations of the content of the Elasticsearch index.

The GeoNetwork community has prepared some sample visualisations. Load the samples from [export.ndjson](https://github.com/geonetwork/core-geonetwork/blob/4.0.x/es/es-dashboards/data/export.ndjson) via the **Saved objects --> Import** option in Kibana.
...
Read more about the usage of Kibana at the [Kibana introduction](https://www.elastic.co/guide/en/kibana/current/introduction.html).
...
GeoNetwork includes a representation of the sitemap record in [schema.org](https://schema.org/) (Structured Data for the Web) framework:

 *   Published metadata record accessed through sitemap / GeoNetwork API as HTML include [`json-ld`](https://en.wikipedia.org/wiki/JSON-LD) linked data (embedded JSON in the HTML page as a script element). 

 *   The included `json-ld` uses vocabulary from schema.org and also the [Croissant extension](https://docs.mlcommons.org/croissant/) for machine learning.
     ...
     Croissant is used to cover data model aspects (ISO19110) and additional details about distributions not covered by schema.org.
