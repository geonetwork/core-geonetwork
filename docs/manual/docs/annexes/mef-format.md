...
### Nested folders in public and private directories

Files in the public and private directories may be organised into subfolders (eg. `public/images/thumbnail.png`). A reader must recursively walk both directories rather than assume a flat file listing. The file's name, as registered in info.xml (see below), is its path relative to the public or private directory, with `/` as the separator regardless of the writer's operating system.
...
4.  *store* (version 3.0 and above): A single container listing every file in both the public and private directories (thumbnails, maps, and any other uploaded document). This container contains a file element for each file. Mandatory attributes of this element are name, which represents the file's name (its path relative to the public or private directory, see [Nested folders](#nested-folders-in-public-and-private-directories)), changeDate, which contains the date of the latest change to the file, and access, whose value must belong to the following set: { *public*, *private* } and indicates which directory the file belongs to. The mimetype attribute is optional; when present, it records the file's content type. The store element is optional but, if present, must list all the files present in the metadata's public and private directories, and any reader that imports these files must set the latest change date and, if provided, mimetype on these using the provided values. The purpose of this element is to provide more information in the case the MEF format is used for metadata harvesting.

    Prior to version 3.0, this information was split across two separate elements, public and private, described below. A reader capable of reading version 3.0 must continue to support archives that use the pre-3.0 form, since the version policy in the introduction to this section only guarantees forward compatibility for readers, not that every writer has moved to the newer minor version.
