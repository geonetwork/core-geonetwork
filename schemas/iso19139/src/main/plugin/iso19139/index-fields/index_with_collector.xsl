...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
                ...
                version="2.0"
                xmlns:jicns="java:org.fao.geonet.schema.iso19139.JSONIndexesCollector">
  ...
  <xsl:template match="gmi:MI_Metadata|gmd:MD_Metadata"
    ...
    <xsl:value-of select="jicns:addIndex($jic, 'documentType', 'metadata')" />
    <xsl:value-of select="jicns:addIndex($jic, 'documentStandard', 'iso19139')" />
    <xsl:value-of select="jicns:addIndex($jic, 'uuid', $identifier)" />
    <xsl:value-of select="jicns:addIndex($jic, 'metadataIdentifier', $identifier)" />
    <xsl:for-each select="gmd:metadataStandardName/gco:CharacterString">
      <xsl:value-of select="jicns:addIndex($jic, 'standardName', normalize-space(.))" />
    </xsl:for-each>
    <xsl:value-of select="jicns:addIndex($jic, 'harvestedDate', format-dateTime(current-dateTime(), $dateFormat))" />
    <!-- # Date -->
    ...
    <xsl:for-each select="gmd:dateStamp/*[text() != '' and position() = 1]">
        ...
        <xsl:value-of select="jicns:addIndex($jic, 'dateStamp', translate(string(
    ...
    <!-- # Languages -->
    <xsl:value-of select="jicns:addIndex($jic, 'mainLanguage', $mainLanguage)" />


    <xsl:for-each select="$otherLanguages">
      <xsl:value-of select="jicns:addIndex($jic, 'otherLanguage', .)" />
    </xsl:for-each>
    ...
    <xsl:choose>
      <xsl:when test="$isDataset">
        <xsl:value-of select="jicns:addIndex($jic, 'resourceType', 'dataset')" />
      </xsl:when>
      <xsl:otherwise>
        <xsl:for-each select="gmd:hierarchyLevel/gmd:MD_ScopeCode/
                              @codeListValue[normalize-space(.) != '']">
          <xsl:value-of select="jicns:addIndex($jic, 'resourceType', .)" />
        </xsl:for-each>
      </xsl:otherwise>
    </xsl:choose>
    ...
    <doc>
      ...
      <xsl:value-of select="jicns:addIndex($jic, 'document', '')" />
      ...
      <xsl:for-each select=".//*[@codeListValue != '' and
        ...
        <xsl:value-of select="jicns:addIndex($jic, concat('codelist_', local-name(..)), @codeListValue)" />
      </xsl:for-each>
      ...
      <xsl:for-each select="gmd:identificationInfo[1]/*[1]">
        <xsl:for-each select="gmd:citation/gmd:CI_Citation">
          <xsl:value-of select="jicns:addIndex($jic, 'resourceTitle', gmd:title/gco:CharacterString/text())" />
          <xsl:value-of select="jicns:addIndex($jic, 'resourceAltTitle', gmd:alternateTitle/gco:CharacterString/text())" />
          ...
          <xsl:for-each select="gmd:date/gmd:CI_Date[gmd:date/*/text() != '' and
            ...
            <xsl:value-of select="jicns:addIndex($jic, concat($dateType, 'DateForResource'), $date)" />
            <xsl:value-of select="jicns:addIndex($jic, concat($dateType, 'YearForResource'), substring($date, 0, 5))" />
            <xsl:value-of select="jicns:addIndex($jic, concat($dateType, 'MonthForResource'), substring($date, 0, 8))" />
          </xsl:for-each>

          <xsl:for-each select="gmd:presentationForm/gmd:CI_PresentationFormCode/@codeListValue[. != '']">
            <xsl:value-of select="jicns:addIndex($jic, 'presentationForm', .)" />
          </xsl:for-each>
        </xsl:for-each>
        <xsl:value-of select="jicns:addIndex($jic, 'resourceAbstract', normalize-space(substring(
        ...
        <xsl:for-each select="gmd:credit/*[. != '']">
          <xsl:value-of select="jicns:addIndex($jic, 'resourceCredit', .)" />
        </xsl:for-each>
        ...
        <xsl:value-of select="jicns:addIndex($jic, 'hasOverview', if (count($overviews) > 0) then 'true' else 'false')" />

        <xsl:for-each select="$overviews">
          <xsl:value-of select="jicns:addIndex($jic, 'overviewUrl', normalize-space(.))" />
        </xsl:for-each>

        <xsl:for-each
          select="gmd:language/gco:CharacterString|gmd:language/gmd:LanguageCode/@codeListValue">
          <xsl:value-of select="jicns:addIndex($jic, 'resourceLanguage', .)" />
        </xsl:for-each>
        ...
        <xsl:for-each
          ...
          <xsl:for-each select="gco:CharacterString[. != '']|
            ...
            <xsl:variable name="inspireTheme" as="xs:string"
                          select="text()"/>
            <xsl:value-of select="jicns:addIndex($jic, 'inspireTheme_syn', text())" />
            <xsl:value-of select="jicns:addIndex($jic, 'inspireTheme', $inspireTheme)" />
            ...
            <xsl:if test="$position = 1">
              <xsl:value-of select="jicns:addIndex($jic, 'inspireThemeFirst_syn', text())" />
              <xsl:value-of select="jicns:addIndex($jic, 'inspireThemeFirst', $inspireTheme)" />
              <xsl:value-of select="jicns:addIndex($jic, 'inspireAnnexForFirstTheme', $inspireTheme)" />
            </xsl:if>
            <xsl:value-of select="jicns:addIndex($jic, 'inspireAnnex', text())" />
          </xsl:for-each>
        </xsl:for-each>
        ...
        <xsl:value-of select="jicns:addIndex($jic, 'inspireThemeNumber', count($inspireKeywords))" />
        <xsl:value-of select="jicns:addIndex($jic, 'hasInspireTheme', if (count($inspireKeywords) > 0) then 'true' else 'false')" />
        ...
        <xsl:value-of select="jicns:addIndex($jic, 'tagNumber', count($keywords))" />


        <xsl:for-each select="$keywords">
          <xsl:value-of select="jicns:addIndex($jic, 'tag', text())" />
        </xsl:for-each>
        ...
        <xsl:choose>
          <xsl:when test="normalize-space($isOpenData) != ''">
            <xsl:value-of select="jicns:addIndex($jic, 'isOpenData', 'true')" />
          </xsl:when>
          <xsl:otherwise>
            <xsl:value-of select="jicns:addIndex($jic, 'isOpenData', 'false')" />
          </xsl:otherwise>
        </xsl:choose>
        ...
        <xsl:for-each
          ...
          <xsl:value-of select="jicns:addIndex($jic, 'geotag', text())" />
        </xsl:for-each>
        ...
        <xsl:for-each
          ...
          <xsl:if test="normalize-space($key) != ''">
            ...
            <xsl:for-each select="*[normalize-space() != '']|
              ...
              <xsl:value-of select="jicns:addIndex($jic, concat('thesaurus_', replace($key, '[^a-zA-Z0-9]', '')), normalize-space(.))" />
            </xsl:for-each>
          </xsl:if>
        </xsl:for-each>
        ...
        <xsl:for-each select="gmd:topicCategory/gmd:MD_TopicCategoryCode">
          <xsl:value-of select="jicns:addIndex($jic, 'topic', .)" />
        ...
        <xsl:for-each select="gmd:spatialResolution/gmd:MD_Resolution">
          <xsl:for-each
            ...
            <xsl:value-of select="jicns:addIndex($jic, 'resolutionScaleDenominator', .)" />
          </xsl:for-each>

          <xsl:for-each select="gmd:distance/gco:Distance[. != '']">
            <xsl:value-of select="jicns:addIndex($jic, 'resolutionDistance', concat(., ' ', @uom))" />
          </xsl:for-each>
        </xsl:for-each>

        <xsl:for-each
          select="gmd:spatialRepresentationType/gmd:MD_SpatialRepresentationTypeCode/@codeListValue[. != '']">
          <xsl:value-of select="jicns:addIndex($jic, 'spatialRepresentationType', .)" />
        </xsl:for-each>


        <xsl:for-each select="gmd:resourceConstraints">
          <xsl:for-each select="*/gmd:accessConstraints/gmd:MD_RestrictionCode/@codeListValue[. != '']">
            <xsl:value-of select="jicns:addIndex($jic, 'accessConstraints', .)" />
          </xsl:for-each>
          <xsl:for-each select="*/gmd:otherConstraints/gco:CharacterString[. != '']">
            <xsl:value-of select="jicns:addIndex($jic, 'otherConstraints', .)" />
          </xsl:for-each>
          <xsl:for-each select="*/gmd:classification/gmd:MD_ClassificationCode/@codeListValue[. != '']">
            <xsl:value-of select="jicns:addIndex($jic, 'constraintClassification', .)" />
          </xsl:for-each>
          <xsl:for-each select="*/gmd:useLimitation/gco:CharacterString[. != '']">
            <xsl:value-of select="jicns:addIndex($jic, 'useLimitation', .)" />
          </xsl:for-each>
        </xsl:for-each>
        ...
        <xsl:for-each select="*/gmd:EX_Extent">
          ...
          <xsl:for-each select="gmd:geographicElement/gmd:EX_GeographicDescription/
            ...
            <xsl:value-of select="jicns:addIndex($jic, 'geoTag', .)" />
          </xsl:for-each>
              ...
              <xsl:when test="-180 &lt;= number($e) and number($e) &lt;= 180 and
                ...
                <xsl:choose>
                  <xsl:when test="$e = $w and $s = $n">
                    <xsl:value-of select="jicns:addIndex($jic, 'location', concat($s, ',', $w))" />
                  </xsl:when>
                  ...
                  <xsl:otherwise>
                    ...
                    <xsl:value-of select="jicns:addIndex($jic, 'geom', $geom)" />
                    <xsl:value-of select="jicns:addIndex($jic, 'location', concat(
        ...
        <xsl:for-each select="srv:serviceType/gco:LocalName">
          <xsl:value-of select="jicns:addIndex($jic, 'serviceType', text())" />

          <xsl:variable name="inspireServiceType" as="xs:string"
                        select="text()"/>
          <xsl:if test="$inspireServiceType != ''">
            <xsl:value-of select="jicns:addIndex($jic, 'inspireServiceType', lower-case($inspireServiceType))" />
          </xsl:if>
          <xsl:if test="following-sibling::srv:serviceTypeVersion">
            <xsl:value-of select="jicns:addIndex($jic, 'serviceTypeAndVersion', concat(
                        text(),
                        $separator,
      ...
      <xsl:for-each select="gmd:referenceSystemInfo/gmd:MD_ReferenceSystem">
        <xsl:for-each select="gmd:referenceSystemIdentifier/gmd:RS_Identifier">
          ...
          <xsl:if test="$crs != ''">
            <xsl:value-of select="jicns:addIndex($jic, 'coordinateSystem', $crs)" />
          </xsl:if>
        </xsl:for-each>
      </xsl:for-each>
      ...
      <xsl:choose>
        ...
        gmd:title/gco:CharacterString">
            ...
            <xsl:if test="count($matchingEUText) = 1">
              ...
              <xsl:value-of select="jicns:addIndex($jic, 'inspireConformResource', $pass)" />
            </xsl:if>
          </xsl:for-each-group>
        </xsl:when>
        <xsl:otherwise>
      ...
      <xsl:for-each-group select="gmd:dataQualityInfo/*/gmd:report"
        ...
        <xsl:if test="$pass">
          <xsl:value-of select="jicns:addIndex($jic, concat('conformTo_', replace(normalize-space($title), '[^a-zA-Z0-9]', '')), $pass)" />
        </xsl:if>
      </xsl:for-each-group>
      ...
      <xsl:for-each select="gmd:dataQualityInfo/*">
        ...
        <xsl:for-each select="gmd:lineage/gmd:LI_Lineage/gmd:statement/gco:CharacterString[. != '']">
          <xsl:value-of select="jicns:addIndex($jic, 'lineage', normalize-space(.))" />
        </xsl:for-each>
        ...
        <xsl:for-each select="gmd:report/*[
          ...
          <xsl:for-each select="gmd:result/gmd:DQ_QuantitativeResult/gmd:value">
            <xsl:if test=". != ''">
              <xsl:value-of select="jicns:addIndex($jic, concat('measure_', replace($measureName, '[^a-zA-Z0-9]', '')), .)" />
            </xsl:if>
          </xsl:for-each>
        </xsl:for-each>
      </xsl:for-each>
      ...
      <xsl:for-each select="gmd:distributionInfo/*">
        <xsl:for-each
          ...
          <xsl:value-of select="jicns:addIndex($jic, 'format', .)" />
        </xsl:for-each>
        ...
        <xsl:for-each select="gmd:transferOptions/*/
          ...
          <xsl:value-of select="jicns:addIndex($jic, 'linkUrl', normalize-space(gmd:linkage/gmd:URL))" />
          <xsl:value-of select="jicns:addIndex($jic, 'linkProtocol', $protocol)" />
      ...
      <xsl:for-each
        ...
        <xsl:if test="$datasetId != ''">
          <xsl:value-of select="jicns:addIndex($jic, 'recordOperateOn', $datasetId)" />
        </xsl:if>
      </xsl:for-each>
  ...
  <xsl:template mode="index-contact" match="*[gmd:CI_ResponsibleParty]">
    ...
    <xsl:if test="normalize-space($organisationName) != ''">
      <xsl:value-of select="jicns:addIndex($jic, concat('Org', $fieldSuffix), $organisationName)" />
      <xsl:value-of select="jicns:addIndex($jic, concat($role, 'Org', $fieldSuffix), $organisationName)" />
    </xsl:if>
    ...
    <xsl:value-of select="jicns:addIndex($jic, concat('contact', $fieldSuffix), normalize-space($myJson))" />
