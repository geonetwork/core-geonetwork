...
public class PublicationDateAddProcessTest extends XslProcessTest {
    ...
    public void testAddPublicationDate() throws Exception {
        ...
        assertThat(
            inputString, hasXPath("count(//gmd:date[gmd:CI_Date/gmd:dateType/gmd:CI_DateTypeCode/@codeListValue = 'publication'])", equalTo("0")).withNamespaceContext(ns)
        );
        ...
        assertThat(
            resultString, hasXPath("count(//gmd:date[gmd:CI_Date/gmd:dateType/gmd:CI_DateTypeCode/@codeListValue = 'publication'])", equalTo("1")).withNamespaceContext(ns)
        );
        assertThat(
            resultString, hasXPath("//gmd:date[gmd:CI_Date/gmd:dateType/gmd:CI_DateTypeCode/@codeListValue = 'publication']//gco:DateTime/text()", equalTo(newDate)).withNamespaceContext(ns)
        );
    }
    ...
    public void testReplaceMultiplePublicationDates() throws Exception {
        ...
        Element citation = Xml.selectElement(midElement, "gmd:identificationInfo/*/gmd:citation/gmd:CI_Citation", new ArrayList<>(ISO19139SchemaPlugin.allNamespaces));
        Element date2 = new Element("date", citation.getNamespace());
        Element ciDate2 = new Element("CI_Date", citation.getNamespace());
        Element dateVal2 = new Element("date", citation.getNamespace());
        ...
        Element dateType2 = new Element("dateType", citation.getNamespace());
        Element dateTypeCode2 = new Element("CI_DateTypeCode", citation.getNamespace());
        ...
        assertThat(
            midStringWithTwo, hasXPath("count(//gmd:date[gmd:CI_Date/gmd:dateType/gmd:CI_DateTypeCode/@codeListValue = 'publication'])", equalTo("2")).withNamespaceContext(ns)
        );
