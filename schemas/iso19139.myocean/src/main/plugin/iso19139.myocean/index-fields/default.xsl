  ...
  <xsl:for-each select="gmd:distributor/gmd:MD_Distributor/gmd:distributorTransferOptions/gmd:MD_DigitalTransferOptions">
    ...
    <xsl:for-each select="gmd:onLine/gmd:CI_OnlineResource[gmd:linkage/gmd:URL!='']">
      ...
      <xsl:if test="not($wmsLinkNoProtocol)">
        <Field name="link" string="{concat($title, '|', $desc, '|', $linkage, '|', $protocol, '|', $protocol, '|', $tPosition, '|', $applicationProfile)}" store="true" index="false" />
      </xsl:if>
      ...
      <xsl:if test="starts-with($protocol,'OGC:WMS-') and contains($protocol,'-get-map') and string($linkage)!='' and string($title)!=''">
        ...
        <Field name="link"
              ...
              '|application/vnd.google-earth.kml+xml|application/vnd.google-earth.kml+xml', '|', $tPosition, '|', $applicationProfile)}"
