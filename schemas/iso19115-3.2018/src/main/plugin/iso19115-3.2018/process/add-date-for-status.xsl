...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template name="analyze-add-date-for-status">
    ...
    <xsl:variable name="recordStatus"
                  select="$root//mdb:identificationInfo/*/mri:status/*[@codeListValue = $dateTypes/@is]"/>
    ...
    <xsl:for-each select="$recordStatus">
      ...
      <xsl:variable name="hasNoDateForStatus"
                    select="count($root//mdb:identificationInfo/*/mri:citation/*/cit:date/*/cit:dateType/*[@codeListValue = $dateTypeForStatus]) = 0"/>
      ...
      <xsl:if test="$hasNoDateForStatus">
        <suggestion process="add-date-for-status" id="{generate-id()}"
          ...
          <params>{
            "dateTypeForStatus":{"type":"codelist", "codelist": "cit:CI_DateTypeCode", "defaultValue":"<xsl:value-of select="$dateTypeForStatus"/>"},
