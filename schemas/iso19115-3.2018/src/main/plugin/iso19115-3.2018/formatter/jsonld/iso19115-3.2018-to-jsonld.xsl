...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:function name="schema-org-fn:buildColumnId" as="xs:string">
        ...
        <xsl:value-of select="concat($featureTypeId, '/', util:escapeForJson(($carrierOfCharacteristics/gfc:code/gco:CharacterString/text(), string($position))[1]))"/>
    </xsl:function>
    ...
    <!--
    Default to english if no language is set in the metadata.
    -->
    <xsl:variable name="defaultLanguage"
                  select="(($metadata/mdb:MD_Metadata/mdb:defaultLocale/*/lan:language/*/@codeListValue, 'eng')[. != ''])[1]"/>

    <!-- TODO: Convert language code eng > en_US ? -->

    <xsl:variable name="requestedLanguageExist"
                  select="$lang != ''
                        and count($metadata/mdb:MD_Metadata/mdb:otherLocale/*[lan:language/*/@codeListValue = $lang]/@id) > 0"/>
    ...
    <xsl:variable name="requestedLanguageId"
                  select="concat('#', $metadata/mdb:MD_Metadata/mdb:otherLocale/*[lan:language/*/@codeListValue = $requestedLanguage]/@id)"/>
                  ...
                  mode="getJsonLD" match="mdb:MD_Metadata">
    {
        ...
        <xsl:variable name="resourceUri"
                             select="gn-fn-dcat:getResourceUri($metadata)"/>
        ...
        <xsl:for-each select="$dateTypes">
            <xsl:variable name="datesForType"
                          select="$metadata/mdb:identificationInfo/*/mri:citation/*/cit:date[*/cit:dateType/*/@codeListValue = current()/@key]/*/cit:date/*/text()"/>
        ...
        ,"recordSet": [
            <xsl:for-each select="$featureTypes">
                <xsl:variable name="featureTypeId"
                                    select="util:escapeForJson(gfc:typeName/text())"/>
                ...
                "key": [{ "@id": "<xsl:value-of select="schema-org-fn:buildColumnId($featureTypeId, $idColumn, $idColumn/position())"/>" }],
                "field": [
                <xsl:for-each select="gfc:carrierOfCharacteristics/*">
                    {
                        ...
                        <xsl:if test="gfc:definition">,
                            "description":
                            <xsl:apply-templates mode="toJsonLDLocalized" select="gfc:definition"/>
                        </xsl:if>
                        ,"source": {
