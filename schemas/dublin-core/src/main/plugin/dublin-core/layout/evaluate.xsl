...
<xsl:stylesheet version="3.0"
  ...
  <!-- Evaluate an expression. This is schema dependant in order to properly
    ...
    -->
  <xsl:function name="gn-fn-metadata:evaluate-dublin-core">
    ...
    <xsl:try>
      ...
      <xsl:catch>
        <xsl:message>Error evaluating
          <xsl:value-of select="$in"/> in context item <xsl:value-of select="name($base)"/>.
  ...
  <xsl:function name="gn-fn-metadata:evaluate-dublin-core-boolean"
                as="xs:boolean">
    <xsl:param name="base" as="node()"/>
    <xsl:param name="in"/>
