              ...
              <table width="100%">
                <xsl:choose>
                  <xsl:when test="$edit=true() or $currTab!='simple'">
                    <xsl:apply-templates mode="elementEP"
                                         select="gfc:listedValue|geonet:child[string(@name)='listedValue']">
                      <xsl:with-param name="schema" select="$schema"/>
                      <xsl:with-param name="edit" select="$edit"/>
                    </xsl:apply-templates>
                  </xsl:when>
                    ...
                    <xsl:if test="gfc:listedValue">
                      <xsl:call-template name="complexElementGui">
                        <xsl:with-param name="title">
                          <xsl:value-of
                            select="/root/gui/iso19110/element[@name='gfc:listedValue']/label"/>
                          <xsl:text> </xsl:text>
                          (<xsl:value-of
                          select="/root/gui/iso19110/element[@name='gfc:label']/label"/>
                          [<xsl:value-of
                          select="/root/gui/iso19110/element[@name='gfc:code']/label"/>] :
                          <xsl:value-of
                            select="/root/gui/iso19110/element[@name='gfc:definition']/label"/>)
                        </xsl:with-param>
