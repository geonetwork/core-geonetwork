...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template name="editPanel-Z39">
		<div id="z39.editPanel">
			<xsl:call-template name="site-Z39"/>
			<div class="dots"/>
			<xsl:call-template name="options-Z39"/>
			<div class="dots"/>
			<xsl:call-template name="privileges-Z39"/>
			<div class="dots"/>
			<xsl:call-template name="categories-Z39"/>
		</div>
	</xsl:template>

	<!-- ============================================================================================= -->

	<xsl:template name="site-Z39">
		...
		<table border="0">
			<tr>
				...
				<td class="padded"><input id="z39.name" class="content" type="text" value="" size="30"/></td>
			</tr>

			<tr>
				<td class="padded" valign="bottom"><xsl:value-of select="/root/gui/harvesting/icon"/></td>
				<td class="padded">
					<select id="z39.icon" class="content" name="icon" size="1"/>
					&#xA0;
					<img id="z39.icon.image" src="" alt="" />
				</td>
			</tr>
			
			<tr>
				<td class="padded"><xsl:value-of select="/root/gui/harvesting/useAccount"/></td>
				<td class="padded"><input id="z39.useAccount" type="checkbox" checked="on"/></td>
			</tr>

			<tr>
				<td/>
				<td>
					<table id="z39.account">
						<tr>
							<td class="padded"><xsl:value-of select="/root/gui/harvesting/username"/></td>
							<td class="padded"><input id="z39.username" class="content" type="text" value="" size="20"/></td>
						</tr>
		
						<tr>
							<td class="padded"><xsl:value-of select="/root/gui/harvesting/password"/></td>
							<td class="padded"><input id="z39.password" class="content" type="password" value="" size="20"/></td>
						</tr>
					</table>
				</td>
			</tr>			
	...
	<!--xsl:template name="search-CSW">
		...
		<button id="z39.addSearch" class="content" onclick="harvesting.z39.addSearchRow()">
	...
	<xsl:template name="options-Z39">
		...
		<table border="0">
			<tr>
				...
				<td class="padded">
					<input id="z39.every.days"  class="content" type="text" size="2"/> :
					<input id="z39.every.hours" class="content" type="text" size="2"/> :
					<input id="z39.every.mins"  class="content" type="text" size="2"/>
					&#160;
			...
			<tr>
				...
				<td class="padded"><input id="z39.oneRunOnly" type="checkbox" value=""/></td>
			</tr>			
		</table>
	</xsl:template>
	...
	<xsl:template name="privileges-Z39">
		...
		<table>
			<tr>
				...
				<td class="padded"><select id="z39.groups" class="content" size="8" multiple="on"/></td>					
				<td class="padded" valign="top">
					<div align="center">
						<button id="z39.addGroups" class="content" onclick="harvesting.z3950.addGroupRow()">
							<xsl:value-of select="/root/gui/harvesting/add"/>
						</button>
					</div>
				</td>					
	...
	<xsl:template name="categories-Z39">
		<h1 align="left"><xsl:value-of select="/root/gui/harvesting/categories"/></h1>
		
		<select id="z39.categories" class="content" size="8" multiple="on"/>
	</xsl:template>
