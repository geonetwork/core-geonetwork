...
public class QueryBuilderTest extends AbstractThesaurusBasedTest {
    ...
    public void distinct() throws IOException, MalformedQueryException, QueryEvaluationException, AccessDeniedException {
        QueryResultsTable noDistinct = builder().selectId().select(Selectors.LOWER_CORNER, true).build().rawExecute(thesaurus);
        QueryResultsTable distinct = builder().selectId().select(Selectors.LOWER_CORNER, true).distinct(true).build().rawExecute(thesaurus);
