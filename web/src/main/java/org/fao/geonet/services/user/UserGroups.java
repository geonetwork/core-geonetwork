...
public class UserGroups implements Service
{
	...
	public Element exec(Element params, ServiceContext context) throws Exception
	{
		...
		//--- allowed to get this info
				...
				List<Element> adminlist = dbms.select("SELECT groupId FROM UserGroups WHERE userId=? or userId =?  group by groupId having count(*) > 1", Integer.valueOf(myUserId),Integer.valueOf(id)).getChildren();
				if (adminlist.size() == 0) {
