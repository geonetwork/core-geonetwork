 ...
 * inserting/removing/updating metadata schemas for use in GeoNetwork as well as
 * determining whether a metadata record belongs to any of the metadata schemas
 * known to GeoNetwork.
 * 
 * The Schema class holds all information describing a schema in GeoNetwork. The
 * SchemaManager holds a map of Schema objects known to GeoNetwork.
 * 
 */
public class SchemaManager {
    ...
    private static final Namespace GEONET_SCHEMA_PREFIX_NS = Namespace
            .getNamespace("gns", GEONET_SCHEMA_URI);
    private static final Namespace GEONET_SCHEMA_NS = Namespace
            .getNamespace(GEONET_SCHEMA_URI);
     ...
     * 
     * @param basePath
     *            the web app base path
     * @param schemaPluginsCat
     *            the schema catalogue file
     * @param sPDir
     *            the schema plugin directory
     * @param defaultLang
     *            the default language (taken from context)
     * @param defaultSchema
     *            the default schema (taken from config.xml)
     */
    private SchemaManager(String basePath, String schemaPluginsCat,
            String sPDir, String defaultLang, String defaultSchema)
            throws Exception {
        ...
        if (saSchemas == null) {
            Log.error(Geonet.SCHEMA_MANAGER,
                    "Cannot scan plugin schemas directory : "
                            + schemaPluginsDir);
        } else {
            for (int i = 0; i < saSchemas.length; i++) {
                if (!saSchemas[i].equals("CVS")
                        && !saSchemas[i].startsWith(".")) {
                    File schemaDir = new File(this.schemaPluginsDir + FS
                            + saSchemas[i]);
                    if (schemaDir.isDirectory()) {
                        processSchema(schemaPluginsDir + FS, saSchemas[i],
                                schemaPluginCatRoot);
                    }
                }
            }
     ...
     * @throws Exception
     */
    public synchronized static SchemaManager getInstance(String basePath,
            String schemaPluginsCat, String sPDir, String defaultLang,
            String defaultSchema) throws Exception {
        if (schemaManager == null) {
            schemaManager = new SchemaManager(basePath, schemaPluginsCat,
                    sPDir, defaultLang, defaultSchema);
        }
     ...
     * 
     * @param name
     *            the metadata schema we want the MetadataSchema for
     * @return
     */
    public MetadataSchema getSchema(String name) {
        ...
        try {
            ...
            if (schema == null)
                throw new IllegalArgumentException("Schema not registered : "
                        + name);
     ...
     * 
     * @param name
     *            the metadata schema we want to add
     * @param in
     *            stream containing a zip archive of the schema to add
     * @throws Exception
     */
     ...
     * 
     * @param name
     *            the metadata schema we want to update
     * @param in
     *            stream containing a zip archive of the schema to update
     * @throws Exception
     */
    public void updatePluginSchema(String name, InputStream in)
            throws Exception {
        ...
        try {
            ...
            try {
                ...
                String errStr = "Could not update schema "
                        + name
     ...
     * 
     * @param name
     *            the metadata schema we want the directory for
     * @return
     */
    ...
    /**
     * Returns the schema location as a JDOM attribute - this can be either an
     * xsi:schemaLocation or xsi:noNamespaceSchemaLocation depending on the
     * schema.
     * 
     * @param name
     *            the metadata schema we want the schemaLocation for
     * @param context
     * @return
     */
            ...
            if (schemaLoc.equals("")) {
                if (new File(schemaFile).exists()) { // build one
                    ...
                    if (nsUri == null || nsUri.equals("")) {
                        out = new Attribute("noNamespaceSchemaLocation",
                                schemaUrl, Csw.NAMESPACE_XSI);
                    } else {
                        schemaLoc = nsUri + " " + schemaUrl;
                        out = new Attribute("schemaLocation", schemaLoc,
                                Csw.NAMESPACE_XSI);
                    }
                } // else return null - no schema xsd exists - could be dtd
            } else {
                if (nsUri == null || nsUri.equals("")) {
                    out = new Attribute("noNamespaceSchemaLocation", schemaLoc,
                            Csw.NAMESPACE_XSI);
                } else {
                    out = new Attribute("schemaLocation", schemaLoc,
                            Csw.NAMESPACE_XSI);
     ...
     * 
     * @param name
     *            the metadata schema we want the templates directory for
     * @return
     */
     ...
     * 
     * @param name
     *            the metadata schema we want the sample data directory for
     * @return
     */
     ...
     * 
     * @param name
     *            the metadata schema we want the csw present info directory
     * @return
     */
     ...
     * 
     * @param name
     *            the metadata schema we want schema info for
     * @return
     */
    ...
    /**
     * Returns the schema converter elements for a schema (as a list of cloned
     * elements).
     * 
     * @param name
     *            the metadata schema we want search
     * @throws Exception
     *             if schema is not registered
     * @return
     */
     ...
     *            the metadata schema we want search
     * @param namespaceUri
     *            the namespace URI we are looking for
     * @return List of XSLTs that produce this namespace URI (full pathname)
     * @throws Exception
     *             if schema is not registered
     */
    public List<String> existsConverter(String name, String namespaceUri)
            throws Exception {
     ...
     * 
     * @param name
     *            the metadata schema we want to check existence of
     * @return
     */
     ...
     * 
     * @param name
     *            the metadata schema we want to delete - can only be a plugin
     *            schema
     * @throws Exception
     */
     ...
     * 
     * @param name
     *            the metadata schema whose suggestions class we want
     * @return
     */
        ...
        try {
            ...
            if (schema == null)
                throw new IllegalArgumentException(
                        "Schema suggestions not registered : " + name);
    ...
    /**
     * Gets the namespace URI from the schema information (XSD) for the supplied
     * prefix.
     * 
     * @param name
     *            the metadata schema whose namespaces we are searching
     * @param prefix
     *            the namespace prefix we want the URI for
     * @return
     */
        ...
        try {
            ...
            if (schema != null) {
                if (Log.isDebugEnabled(Geonet.SCHEMA_MANAGER))
                    Log.debug(Geonet.SCHEMA_MANAGER, "  => Found schema "
                            + schema
                            + " using AUTODETECT(attributes) examination");
            }
            ...
            if (schema == null) {
                ...
                if (schema != null) {
                    if (Log.isDebugEnabled(Geonet.SCHEMA_MANAGER))
                        Log.debug(Geonet.SCHEMA_MANAGER, "  => Found schema "
                                + schema
                                + " using AUTODETECT(elements) examination");
                }
            }
            ...
            if (schema == null) {
                ...
                if (schema != null) {
                    if (Log.isDebugEnabled(Geonet.SCHEMA_MANAGER))
                        Log.debug(Geonet.SCHEMA_MANAGER, "  => Found schema "
                                + schema
                                + " using AUTODETECT(namespaces) examination");
                }
            }

            // -- If nothing has matched by this point choose defaultSchema
            // supplied
            // -- as argument to this method as long as its reasonable
            if (schema == null && defaultSchema != null) {
                String defaultSchemaOrDependencySchema = checkNamespace(md,
                        defaultSchema);
    ...
    /**
     * Check that schema is present and that the record can be assigned to it -
     * namespace of metadata schema is compared with prime namespace of metadata
     * record.
     * 
     * @param md
     *            the metadata record being checked for prime namespace equality
     * @param schema
     *            the name of the metadata schema we want to test
     * @return
     */
    private String checkNamespace(Element md, String schema) {
        ...
        try {
            ...
            if (mds != null) {
                ...
                if (Log.isDebugEnabled(Geonet.SCHEMA_MANAGER))
                    Log.debug(Geonet.SCHEMA_MANAGER, "  primeNs " + primeNs
                            + " for schema " + schema);
                if (md.getNamespace().getURI().equals(primeNs)) {
                    result = schema;
                } else {
                    // Check if the metadata could match a schema dependency
                    // (If preferredSchema is an ISO profil a fragment or
                    // subtemplate
            ...
            Log.warning(Geonet.SCHEMA_MANAGER, "Schema " + schema
                    + " not registered?");
        }
     ...
     */
    private void realDeletePluginSchema(String name, boolean doDependencies)
            throws Exception {
     ...
     * @throws Exception
     */
    private void realAddPluginSchema(String name, InputStream in)
            throws Exception {
        Element schemaPluginCatRoot = getSchemaPluginCatalog();
     ...
     * 
     * @param in
     *            the InputStream to copy from (usually a zipEntry)
     * @param out
     *            the OutputStream to copy to (eg. file output stream)
     * @throws Exception
     */
     ...
     * 
     * @param dir
     *            the directory into which the zip archive will be unpacked
     * @param in
     *            the schema zip archive
     * @throws Exception
     */
    private void unpackSchemaZipArchive(File dir, InputStream in)
            throws Exception {
        ...
        while (entry != null) {
            ...
            if (entry.isDirectory()) {
                if (Log.isDebugEnabled(Geonet.SCHEMA_MANAGER))
                    Log.debug(Geonet.SCHEMA_MANAGER, "Creating directory "
                            + entry.getName());
                (new File(dir, entry.getName())).mkdir();
            } else {
                if (Log.isDebugEnabled(Geonet.SCHEMA_MANAGER))
                    Log.debug(Geonet.SCHEMA_MANAGER,
                            "Creating file " + entry.getName());
     ...
     * @param xmlSchemaFile
     *            name of XML schema file (usually schema.xsd)
     * @param xmlSuggestFile
     *            name of schema suggestions file
     * @param xmlSubstitutionsFile
     *            name schema substitutions file
     * @param xmlIdFile
     *            name of XML file that identifies the schema
     * @param oasisCatFile
     *            name of XML OASIS catalog file
     * @param conversionsFile
     *            name of XML conversions file
     * @throws Exception
     */
    private void addSchema(String fromAppPath, String name,
            Element schemaPluginCatRoot, String xmlSchemaFile,
            String xmlSuggestFile, String xmlSubstitutionsFile,
            String xmlIdFile, String oasisCatFile, String conversionsFile)
            throws Exception {
        String path = new File(xmlSchemaFile).getParent();

        MetadataSchema mds = new SchemaLoader().load(xmlSchemaFile,
                xmlSubstitutionsFile);
        mds.setName(name);
        ...
        for (String fname : fnames) {
            ...
            if (new File(filePath).exists()) {
                ...
                if (Log.isDebugEnabled(Geonet.SCHEMA_MANAGER))
                    Log.debug(Geonet.SCHEMA_MANAGER,
                            "Adding XmlFile " + Xml.getString(config));
                XmlFile xf = new XmlFile(config, defaultLang, true);
                xfMap.put(fname, xf);
            } else {
                Log.warning(Geonet.SCHEMA_MANAGER, "Unable to load loc file: "
                        + filePath);
            }
        }
        ...
        putSchemaInfo(
                ...
                mds, path + FS, new SchemaSuggestions(xmlSuggestFile),
                extractADElements(xmlIdFile), xfMap,
                true, // all schemas are plugin schemas now
                extractSchemaLocation(xmlIdFile),
                extractConvElements(conversionsFile), extractDepends(xmlIdFile));

        if (Log.isDebugEnabled(Geonet.SCHEMA_MANAGER))
            Log.debug(
                    Geonet.SCHEMA_MANAGER,
                    "Property " + Jeeves.XML_CATALOG_FILES + " is "
                            + System.getProperty(Jeeves.XML_CATALOG_FILES));

        // -- Add entry for presentation xslt to schemaPlugins catalog
        // -- if this schema is a plugin schema
        int baseNrInt = getHighestSchemaPluginCatalogId(name,
                schemaPluginCatRoot);
        if (baseNrInt == 0)
            baseNrInt = numberOfCoreSchemasAdded;
        if (baseNrInt != -1) {
            createUriEntryInSchemaPluginCatalog(name, baseNrInt,
                    schemaPluginCatRoot);
        }
     ...
     * 
     * @param root
     *            the list of elements from the schemaplugin-uri-catalog
     * @param name
     *            the name of the schema to use
     * @return
     * @throws Exception
     */
    private Element deleteSchemaFromPluginCatalog(String name, Element root)
            throws Exception {
        ...
        for (Content content : contents) {
            ...
            if (!uri.getName().equals("uri")
                    || !uri.getNamespace().equals(
                            Geonet.OASIS_CATALOG_NAMESPACE)) {
                if (Log.isDebugEnabled(Geonet.SCHEMA_MANAGER))
                    Log.debug(Geonet.SCHEMA_MANAGER,
                            "Skipping element " + uri.getQualifiedName() + ":"
                                    + uri.getNamespace());
                continue;
            }
     ...
     * Geonet.File.METADATA_MAX_BLANKS). If the presentation xslt is already
     * mapped then we exit early with return value -1.
     * 
     * @param root
     *            the list of elements from the schemaplugin-uri-catalog
     * @param name
     ...
     * @throws Exception
     */
    private int getHighestSchemaPluginCatalogId(String name, Element root)
            throws Exception {
        ...
        try {
            ...
            nfe.printStackTrace();
            throw new IllegalArgumentException(
                    "Cannot decode blank number from " + baseBlank);
        }
        return baseNrInt;
    }
     ...
     *            the name of the schema to use
     * @param baseNrInt
     *            the number of the plugin schema to map the presentation xslt
     *            to
     * @param root
     *            the list of elements from the schemaplugin-uri-catalog
     * @throws Exception
     */
    private void createUriEntryInSchemaPluginCatalog(String name,
            int baseNrInt, Element root) throws Exception {
        baseNrInt = baseNrInt + 1;

        Element newBlank = new Element("rewriteURI",
                Geonet.OASIS_CATALOG_NAMESPACE);
        // Element newBlank = new Element("uri",
        // Geonet.OASIS_CATALOG_NAMESPACE);
        if (baseNrInt <= Geonet.File.METADATA_MAX_BLANKS) {
            String zero = "";
            if (baseNrInt < 10)
                zero = "0";
            newBlank.setAttribute("uriStartString", Geonet.File.METADATA_BLANK
                    + zero + baseNrInt);
            newBlank.setAttribute("rewritePrefix", buildSchemaFolderPath(name));
        } else {
            throw new IllegalArgumentException(
                    "Exceeded maximum number of plugin schemas "
                            + Geonet.File.METADATA_MAX_BLANKS);
        }
     ...
     *            schema name
     * @param mds
     *            MetadataSchema object with details of XML schema info
     * @param schemaDir
     *            path name of schema directory
     * @param sugg
     *            SchemaSuggestions object
     * @param adElems
     *            List of autodetect XML elements (as JDOM Elements)
     * @param xfMap
     *            Map containing XML localized info files (as Jeeves XmlFiles)
     * @param isPlugin
     *            true if schema is a plugin schema
     * @param schemaLocation
     *            namespaces and URLs of their xsds
     * @param convElems
     *            List of elements in conversion file
     * @param dependElems
     *            List of depend XML elements (as JDOM Elements)
     */
    private void putSchemaInfo(String name, String id, String version,
            MetadataSchema mds, String schemaDir, SchemaSuggestions sugg,
            List<Element> adElems, Map<String, XmlFile> xfMap,
     ...
     * directory itself and the mapping for the schema presentation xslt from
     * the schemaplugins oasis catalog.
     * 
     * @param name
     *            schema name
     * @throws Exception
    ...
    private void removeSchemaInfo(String name) throws Exception {
        ...
        Element schemaPluginCatRoot = getSchemaPluginCatalog();
        schemaPluginCatRoot = deleteSchemaFromPluginCatalog(name,
                schemaPluginCatRoot);
        writeSchemaPluginCatalog(schemaPluginCatRoot);
    ...
    /**
     * Deletes information in the schema directory and removes published schemas
     * from the webapp.
     * 
     * @param dir
     *            schema directory to remove
     * @param name
     *            of schema being removed
     */
    private void removeSchemaDir(String dir, String name) {
        // -- FIXME: get schema directory and zip it up into the deleted
        // metadata
        // -- directory?
        ...
        if (Log.isDebugEnabled(Geonet.SCHEMA_MANAGER))
            Log.debug(Geonet.SCHEMA_MANAGER, "Delete operation returned "
                    + deleteOp);

        String pubSchemaDir = basePath + Geonet.Path.SCHEMAS + name;
        Log.debug(Geonet.SCHEMA_MANAGER,
                "Removing published schemas directory " + pubSchemaDir);
     ...
     * 
     * @param schemasDir
     *            path name of directory containing schemas
     * @param saSchema
     *            schema in schemasDir to process
     * @param schemaPluginCatRoot
     * @return
     */
    private void processSchema(String schemasDir, String saSchema,
            Element schemaPluginCatRoot) throws OperationAbortedEx {

        Log.info(Geonet.SCHEMA_MANAGER, "    Adding xml schema : " + saSchema);
        String schemaFile = schemasDir + saSchema + "/" + Geonet.File.SCHEMA;
        String suggestFile = schemasDir + saSchema + "/"
                + Geonet.File.SCHEMA_SUGGESTIONS;
        String substitutesFile = schemasDir + saSchema + "/"
                + Geonet.File.SCHEMA_SUBSTITUTES;
        String idFile = schemasDir + saSchema + "/" + Geonet.File.SCHEMA_ID;
        String oasisCatFile = schemasDir + saSchema + "/"
                + Geonet.File.SCHEMA_OASIS;
        String conversionsFile = schemasDir + saSchema + "/"
                + Geonet.File.SCHEMA_CONVERSIONS;
        ...
        try {
            ...
            if (hmSchemas.containsKey(saSchema)) { // exists so ignore it
                Log.error(Geonet.SCHEMA_MANAGER, "Schema " + saSchema
                        + " already exists - cannot add!");
            } else {
                stage = "adding the schema information";
                addSchema(schemasDir, saSchema, schemaPluginCatRoot,
                        schemaFile, suggestFile, substitutesFile, idFile,
                        oasisCatFile, conversionsFile);
            }
        } catch (Exception e) {
            String errStr = "Failed whilst " + stage
                    + ". Exception message if any is " + e.getMessage();
            Log.error(Geonet.SCHEMA_MANAGER, errStr);
            e.printStackTrace();
            throw new OperationAbortedEx(errStr, e);
        }
     ...
     * 
     */
    private void checkDependencies(Element schemaPluginCatRoot)
            throws Exception {
        ...
        for (String schemaName : hmSchemas.keySet()) {
            ...
            try {
                ...
                Log.error(Geonet.SCHEMA_MANAGER, "check dependencies failed: "
                        + e.getMessage());
     ...
     *            Schema being checked
     * @param dependsList
     *            List of depend elements for schema.
     * @throws Exception
     */
    private void checkDepends(String thisSchema, List<Element> dependsList)
            throws Exception {
        ...
        for (Element depends : dependsList) {
            ...
            if (schema.length() > 0) {
                if (!hmSchemas.containsKey(schema)) {
                    throw new IllegalArgumentException("Schema " + thisSchema
                            + " depends on " + schema
                            + ", but that schema is not loaded");
                }
            }
        }
    }
     ...
     * 
     * @param xmlIdFile
     *            name of schema XML identification file
    ...
    private List<Element> extractDepends(String xmlIdFile) throws Exception {
        ...
        List<Element> dependsList = root.getChildren("depends",
                GEONET_SCHEMA_NS);
    ...
    /**
     * Extract schema version and uuid info from identification file and compare
     * specified name with name in identification file.
     * 
     * @param xmlIdFile
     *            name of schema XML identification file
     * @param name
     ...
     * @throws Exception
     */
    private Pair<String, String> extractIdInfo(String xmlIdFile, String name)
            throws Exception {
     ...
     * 
     * @param xmlConvFile
     *            name of schema XML conversions file
     * @return
     * @throws Exception
     */
    private List<Element> extractConvElements(String xmlConvFile)
            throws Exception {
        List<Element> result = new ArrayList<Element>();
        if (!(new File(xmlConvFile).exists())) {
            if (Log.isDebugEnabled(Geonet.SCHEMA_MANAGER))
                Log.debug(Geonet.SCHEMA_MANAGER,
                        "Schema conversions file not present");
        } else {
            Element root = Xml.loadFile(xmlConvFile);
            if (root.getName() != "conversions")
                throw new IllegalArgumentException("Schema conversions file "
                        + xmlConvFile
    ...
    private String extractSchemaLocation(String xmlIdFile) throws Exception {
        Element root = Xml.loadFile(xmlIdFile);
        Element schemaLocElem = root.getChild("schemaLocation",
                GEONET_SCHEMA_NS);
        if (schemaLocElem == null)
            schemaLocElem = root.getChild("schemaLocation",
                    GEONET_SCHEMA_PREFIX_NS);
        return schemaLocElem.getText();
    }

    /**
     * Search all available schemas for one which contains the element(s) or
     * attributes specified in the autodetect info.
     * 
     * @param md
     *            the XML record whose schema we are trying to find
     * @param mode
     * @return
     * @throws org.fao.geonet.exceptions.SchemaMatchConflictException
     */
    private String compareElementsAndAttributes(Element md, int mode)
        ...
        if (Log.isDebugEnabled(Geonet.SCHEMA_MANAGER))
            Log.debug(Geonet.SCHEMA_MANAGER,
                    "Schema autodetection starting on " + md.getName()
        ...
        for (String schemaName : allSchemas) {
            ...
            for (Element elem : adElems) {
                ...
                if (mode == MODE_ATTRIBUTEWITHVALUE
                        && elem.getName() == "attributes") {
                ...
                } else if (mode == MODE_NAMESPACE
                        && elem.getName() == "namespaces") {
                    List<Namespace> nss = elem.getAdditionalNamespaces();
                    for (Namespace ns : nss) {
                        if (Log.isDebugEnabled(Geonet.SCHEMA_MANAGER))
                            Log.debug(Geonet.SCHEMA_MANAGER,
                                    "				Finding namespace " + ns.toString());
                    ...
                    for (Element kid : elemKids) {
                        ...
                        if (mode == MODE_ROOT && type != null
                                && "root".equals(type.getValue())) {
                            if (Log.isDebugEnabled(Geonet.SCHEMA_MANAGER))
                                Log.debug(
                                        Geonet.SCHEMA_MANAGER,
                                        "				Comparing "
                            ...
                            // --- try and find the kid in the md (kid only, not
                            // value)
        ...
        if (matches.size() > 1) {
            throw new SchemaMatchConflictException("Metadata record with "
                    + md.getName() + " (Namespace " + md.getNamespace()
     ...
     * matches the "needle" metadata attribute arg - A matching attribute has
     * the same name and value.
     * 
     * @param needle
     *            the XML attribute we are trying to find
     * @param haystack
     *            the XML metadata record we are searching
     * @return
     */
    private boolean isMatchingAttributeInMetadata(Attribute needle,
            Element haystack) {
        boolean returnVal = false;
        Iterator<Element> haystackIterator = haystack
     ...
     * matches the "needle" namespace arg. (Note: matching namespaces have the
     * same URI, prefix is ignored).
     * 
     * @param needle
     *            the XML namespace we are trying to find
     * @param haystack
     *            the XML metadata record we are searching
     * @return
     */
    private boolean isMatchingNamespaceInMetadata(Namespace needle,
            Element haystack) {
        if (Log.isDebugEnabled(Geonet.SCHEMA_MANAGER))
            Log.debug(Geonet.SCHEMA_MANAGER, "Matching " + needle.toString());
    ...
    /**
     * This method searches an elements and its namespaces for a match with an
     * input namespace.
     * 
     * @param ns
     *            the XML namespace we are trying to find
     * @param elem
     *            the XML metadata element whose namespaces are to be searched
     * @return
     */
    ...
    /**
     * This method searches an entire metadata file for an element that matches
     * the "needle" metadata element arg - A matching element has the same name,
     * namespace and value.
     * 
     * @param needle
     *            the XML element we are trying to find
     * @param haystack
     *            the XML metadata record we are searching
     * @param checkValue
     *            compare the value of the needle with the value of the element
     *            we find in the md
     * @return
     */
    private boolean isMatchingElementInMetadata(Element needle,
            Element haystack, boolean checkValue) {
        boolean returnVal = false;
        Iterator<Element> haystackIterator = haystack
                .getDescendants(new ElementFilter());
     ...
     * 
     * @param dir
     *            the dir whose contents are to be deleted
     * @return
     */
     ...
     * 
     * @param context
     *            the ServiceContext used to get setting manager and appPath
     * @param schemaName
    ...
    /**
     * Copy the schema.xsd file and the schema directory from the schema plugin
     * directory to the webapp.
     * 
     * @param name
     *            the name of the schema
     * @param schemaPluginDir
     *            the directory containing the schema plugin
     */
    private void copySchemaXSDsToWebApp(String name, String schemaPluginDir)
            throws Exception {
        ...
        if (schemaFiles.length > 0) {
            ...
            Log.error(Geonet.SCHEMA_MANAGER, "Schema " + name
                    + " does not have any XSD files!");
        }

        File fileSchemaDir = new File(schemaPluginDir, "schema");
        if (fileSchemaDir.exists()) {
            FileCopyMgr.copyFiles(fileSchemaDir, new File(webAppDirSchemaXSD,
                    "schema"));
        }
