...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template mode="permitMarkup-iso19139.sdn-product"
		match="gmd:abstract | gmd:description | gmd:statement | gmd:purpose | gmd:supplementalInformation">
		<xsl:value-of select="true()"/>
	</xsl:template>
	<xsl:template mode="permitMarkup-iso19139.sdn-product" match="*">
		<xsl:value-of select="false()"/>
	</xsl:template>
	
	<xsl:template name="permitMarkup-iso19139.sdn-product">
		<xsl:apply-templates mode="permitMarkup-iso19139.sdn-product" select="."/>
	</xsl:template>
</xsl:stylesheet>
