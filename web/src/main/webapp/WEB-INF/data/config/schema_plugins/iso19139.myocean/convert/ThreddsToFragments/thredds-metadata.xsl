...
<xsl:stylesheet version="2.0"
                ...
                <record uuid="{$uuid}">
                    ...
                    <fragment id="thredds.extent" uuid="{util:toString(util:randomUUID())}"
                        ...
                        <gmd:extent>
                            <gmd:EX_Extent>
                                ...
                                <xsl:for-each
                                    select="tds:geospatialCoverage|tds:metadata//tds:geospatialCoverage">
                    ...
                    <replacementGroup id="thredds.online.resources">
                        ...
                        <xsl:if test="not(tds:dataset)">
                            <xsl:for-each
                                select="tds:serviceName|tds:metadata//tds:serviceName|@serviceName">
                                <xsl:variable name="serviceName" select="string(.)"/>
                                <xsl:apply-templates
                                    select="/root/tds:catalog//tds:service[@name=$serviceName]"
                    ...
                    <replacementGroup id="thredds.maintenance">
                        <xsl:if
                            test="tds:property[@name='update-schedule']|tds:metadata//tds:property[@name='update-schedule']">
                            <fragment uuid="{util:toString(util:randomUUID())}"
                                title="{concat($datasetName,'_maintenance')}">
    ...
    <xsl:template match="tds:keyword" mode="keywords-cf">
        <xsl:variable name="cf-skos" select="'../../../../../../../../../geonetwork/data/config/codelist/local/thesauri/parameter/myocean.ocean-variables.rdf'"/>
        <xsl:variable name="cf-param" select="."/>
        <xsl:variable name="cf-href" select="document($cf-skos)/rdf:RDF/skos:Collection/skos:member/skos:Concept[skos:prefLabel/text()=$cf-param]/@rdf:about"/>
        <xsl:choose>
    ...
    <xsl:template match="tds:service" mode="transfer-options">
        <xsl:choose>
            ...
            <xsl:otherwise>
                ...
                <xsl:variable name="ID" select="/root/tds:catalog/tds:dataset/@ID"/>
                ...
                <xsl:variable name="protocol">
                    <xsl:choose>
                        ...
                        <xsl:when test="upper-case(@serviceType)='FTP'">WWW:FTP</xsl:when>
                        <xsl:when test="upper-case(@serviceType)='MAIL'">WWW:MAIL</xsl:when>
                        ...
                        <xsl:otherwise>WWW:LINK</xsl:otherwise>
                    </xsl:choose>
                </xsl:variable>
                                ...
                                <gmd:URL>
                                    <xsl:choose>
                                        <xsl:when test="@serviceType='WMS'">
                                            <xsl:value-of
                                                select="concat($catalogHost,$baseUrl,$urlPath,'?service=WMS&amp;version=1.3.0&amp;request=GetCapabilities')"
                                            />
                                        </xsl:when>
                                        <xsl:when test="@serviceType='WCS'">
                                            <xsl:value-of
                                                select="concat($catalogHost,$baseUrl,$urlPath,'?service=WCS&amp;version=1.0.0&amp;request=GetCapabilities')"
                                            />
                                        </xsl:when>
                                        ...
                                        <xsl:when test="upper-case(@serviceType)='MOTU-DGF'">
                                            <xsl:value-of
                                                select="concat($baseUrl,'http%3A%2F%2Fpurl.org%2Fmyocean%2Fontology%2Fproduct%2Fdatabase%23',$ID)"
                                            />
                                        </xsl:when>
                                        <xsl:otherwise>
                                            <xsl:value-of
