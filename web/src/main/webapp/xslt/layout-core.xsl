...
<xsl:stylesheet version="2.0" 
  ...
  <xsl:template name="render-element">
    ...
    <xsl:variable name="isRequired" as="xs:boolean">
      <xsl:choose>
        <xsl:when test="$isEditing">
          <xsl:choose>
            <xsl:when test="$parentEditInfo and $parentEditInfo/@min = 1 and $parentEditInfo/@max = 1"><xsl:value-of select="true()"/></xsl:when>
            <xsl:when test="not($parentEditInfo) and $editInfo and $editInfo/@min = 1 and $editInfo/@max = 1"><xsl:value-of select="true()"/></xsl:when>
    ...
    <label for="gn-TODO" class="{if ($isRequired) then 'gn-required' else ''}">
    ...
    <xsl:choose>
      <xsl:when test="$type = 'textarea'">
        <textarea name="_{$name}">
          ...
          <xsl:value-of select="$valueToEdit"/></textarea>
      </xsl:when>
      <xsl:when test="$type = 'select'">
        <select name="_{$name}">
          ...
          <option value="{$valueToEdit}"><xsl:value-of select="$value"/></option>
        </select>
        
      </xsl:when>
      <xsl:otherwise>
        <input name="_{$name}" value="{$valueToEdit}">
          <xsl:if test="$isRequired">
            <xsl:attribute name="required" select="'required'"/>
          </xsl:if>
          <xsl:if test="$type != ''">
    ...
    <div id="navbarExample" class="navbar navbar-static navbar-fixed-bottom">
      <div class="navbar-inner">
        <div class="container" style="width: auto;">
          <ul class="nav">
            <li class="active"><a href="#identificationInfo">@gmd:identificationInfo</a></li>
            <li><a href="#spatialRepresentationInfo">@gmd:spatialRepresentationInfo</a></li>
            <li><a href="#distributionInfo">@gmd:distributionInfo</a></li>
            <li><a href="#dataQualityInfo">@gmd:dataQualityInfo</a></li>
          </ul>
        </div>
      </div>
    </div>
