...
<xsl:stylesheet version="2.0" xmlns:xs="http://www.w3.org/2001/XMLSchema"
  ...
  <xsl:template name="render-boxed-element-control">
    ...
    <button class="btn icon-remove btn-danger pull-right"
  ...
  <xsl:template name="render-element-to-add">
      ...
      <div class="col-lg-10">
        <xsl:choose>
          ...
          <xsl:when test="$childEditInfo/gn:choose">
            <div class="btn-group">
              <button type="button" class="btn dropdown-toggle icon-plus"
              ...
              <ul class="dropdown-menu">
                <xsl:for-each select="$childEditInfo/gn:choose">
                   <xsl:message><xsl:copy-of select="$childEditInfo"></xsl:copy-of></xsl:message>
                  <li><a 
          ...
          <xsl:otherwise>
            <i class="btn icon icon-plus" 
              data-ng-click="add({$parentEditInfo/@ref}, '{concat(@prefix, ':', @name)}', '{$id}', 'before');"></i>
          </xsl:otherwise>
        </xsl:choose>
    ...
    <xsl:template name="render-form-field">
        ...
        <xsl:variable name="valueToEdit" select="if ($value/*) then normalize-space($value/text()) else $value"/>
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                <input class="form-control" id="gn-field-{$editInfo/@ref}" name="_{$name}" value="{$valueToEdit}">
                    <xsl:if test="$isRequired">
                        <xsl:attribute name="required" select="'required'"/>
                    </xsl:if>
                ...
                <xsl:if test="$listOfValues">
                    <xsl:message><xsl:copy-of select="$listOfValues"></xsl:copy-of></xsl:message>
