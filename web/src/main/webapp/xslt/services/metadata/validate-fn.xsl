...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
          ...
          <xsl:analyze-string select="." regex="{$regex}">
            <xsl:matching-substring>
              ...
              <xsl:value-of select="geonet:getTitleWithoutContext($schema, $name, $labels)"/>
                (<xsl:value-of select="$name"/>) </xsl:matching-substring>
  ...
  <xsl:function name="geonet:parse-xsd-error-msg" as="xs:string">
    ...
    <xsl:variable name="rules">
      ...
      <rule errorType="complex-type.2.4.b">The content of element '(.*)' is not complete. One of
        '\{"(.*)\}' is expected\. \(Element: (.*) with parent element: (.*)\)</rule>
      ...
      <rule errorType="datatype-valid.1.2.1">'(.*)' is not a valid value for '(.*)'\. \(Element:
        ([a-z]{3}):(.*) with parent element: (.*)\)</rule>
              ...
              <xsl:choose>
                ...
                <xsl:when test="$errorType = 'complex-type.2.4.b'">
                  ...
                  <xsl:value-of
                    ...
                    select="$labels/validation/isNotComplete"/>
                  <xsl:value-of select="$labels/validation/onElementOf"/>
                ...
                <xsl:when test="$errorType = 'datatype-valid.1.2.1' or $errorType = 'type.3.1.3'">
                  <xsl:value-of select="$labels/validation/invalidValue"/> '<xsl:value-of
                    select="regex-group(1)"/>' <xsl:value-of select="$labels/validation/notValidFor"/>
                  <xsl:value-of
                ...
                <xsl:when test="$errorType = 'enumeration-valid'">
                  <xsl:value-of select="$labels/validation/enum1"/> '<xsl:value-of
                    select="regex-group(1)"/>' <xsl:value-of select="$labels/validation/enum2"/>
                  <xsl:value-of
                    select="geonet:getTitleWithoutContext($schema, concat(regex-group(3), ':', regex-group(4)), $labels)"
                  /> (<xsl:value-of select="concat(regex-group(3), ':', regex-group(4))"/>).
  ...
  <xsl:function name="geonet:parse-xsd-error" as="xs:string">
    ...
    <xsl:analyze-string select="$error" regex=".*cvc-([\w\-0-9\.]+): (.*)">
      <xsl:matching-substring>
        <xsl:value-of
          select="geonet:parse-xsd-error-msg(regex-group(1), regex-group(2), $schema, $labels)"/>
      </xsl:matching-substring>
      <xsl:non-matching-substring>
        <xsl:value-of select="$error"/>
      </xsl:non-matching-substring>
