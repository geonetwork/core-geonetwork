...
<xsl:stylesheet version="1.0" xmlns:gmd="http://www.isotc211.org/2005/gmd"
					...
					<xsl:when test="contains($type,'OPENDAP')">
						...
						<xsl:if test="contains($serverops,'dods')">
							<xsl:call-template name="opendapOps">
								...
								<xsl:with-param name="title" select="'dataset descriptor structure populated with data using a constraint'"/>
							</xsl:call-template>
						</xsl:if>
						<xsl:if test="contains($serverops,'ddx')">
							<xsl:call-template name="opendapOps">
								<xsl:with-param name="op" select="'ddx'"/>
								<xsl:with-param name="title" select="'XML version of dataset descriptor structure (dds) and dataset attribute structure (das)'"/>
							</xsl:call-template>
						</xsl:if>
						<xsl:if test="contains($serverops,'blob')">
							<xsl:call-template name="opendapOps">
								<xsl:with-param name="op" select="'blob'"/>
								<xsl:with-param name="title" select="'Serialized binary data content for requested data set, with a constraint expression applied'"/>
							</xsl:call-template>
						</xsl:if>
						<xsl:if test="contains($serverops,'info')">
							<xsl:call-template name="opendapOps">
								<xsl:with-param name="op" select="'info'"/>
								<xsl:with-param name="title" select="'info object (attributes, types and other information)'"/>
							</xsl:call-template>
						</xsl:if>
	...
	<xsl:template match="*" mode="LayerDataIdentification">
		...
		<gmd:spatialRepresentationType>
			<gmd:MD_SpatialRepresentationTypeCode codeList="./resources/codeList.xml#MD_SpatialRepresentationTypeCode" codeListValue="grid" />
		</gmd:spatialRepresentationType>
