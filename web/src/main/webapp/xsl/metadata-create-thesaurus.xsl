	...
	<xsl:template name="content">
		<xsl:call-template name="formLayout">
			<xsl:with-param name="content">
				<xsl:variable name="thesauriType" select="/root/gui/thesaurusTypes/thesauri/thesaurus[contains(key,/root/request/uuid)]/dname"/>
				...
				<div id="createthesaurus">
					...
					<table width="100%">
							...
							<xsl:choose>
								...
								<xsl:otherwise>
									<td class="padded" align="center" colspan="2">
										...
										<xsl:value-of select="concat(/root/gui/strings/thesaurusType,'  ')"/><b><xsl:value-of select="$thesauriType"/></b><xsl:value-of select="'  already exists for this register'"/>
									</td>
								</xsl:otherwise>
							</xsl:choose>
						</tr>
						<tr>
							<td class="padded" align="center" colspan="2">
								<button class="content" onclick="radioModalUpdate('createthesaurus','thesaurus.add.register',null,'')">
									<xsl:choose>
										<xsl:when test="normalize-space($thesauriType)=''">
