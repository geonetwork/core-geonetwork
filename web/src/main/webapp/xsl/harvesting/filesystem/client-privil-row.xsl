...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="/root/group">
		...
		<tr id="filesystem.group.{@id}">
			...
			<td class="padded" align="center">
				<!--button class="content" onclick="harvesting.filesystem.removeGroupRow('filesystem.group.{@id}')">
					<xsl:value-of select="/root/strings/remove"/>
				</button-->

				<!-- Yet another fix for IE -->

				<a href="javascript:harvesting.filesystem.removeGroupRow('filesystem.group.{@id}')">
					<xsl:value-of select="/root/strings/remove"/>
				</a>		
			</td>
		</tr>
