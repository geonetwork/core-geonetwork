            ...
            <xsl:with-param name="content">
			    <xsl:choose>
			        ...
			        <xsl:when test="/root/response/result='errorProfileRequestFailed'">
			            <p class="error"><xsl:copy-of select="/root/gui/info/errorProfileRequestFailed"/></p>
			        </xsl:when>
            ...
            <xsl:with-param name="buttons">
            	<xsl:if test="not(starts-with(string(/root/response/result),'error'))">
					<xsl:if test="/root/request/simplemetadata">           
						<div align="center">
	                		<button class="content" onclick="doBannerButton('{/root/gui/locService}/simplemetadata.registration.submit?username={/root/response/@username}&amp;email={/root/response/@email}','{/root/gui/strings/simpleMetadataTitle}',800,600)"><xsl:value-of select="/root/gui/strings/submitSimpleRegistration"/></button>
						</div>
					</xsl:if>
				</xsl:if>
            </xsl:with-param>
