...
GeoNetwork.Control.ExtentBox = OpenLayers.Class(OpenLayers.Control, {
    ...
    endBox: function() {
        ...
        if (this.minxelement) {
            this.minxelement.dom.value = minxy.lon.toFixed(4);
        }
        if (this.maxxelement) {
            this.maxxelement.dom.value = maxxy.lon.toFixed(4);
        }
        if (this.minyelement) {
            this.minyelement.dom.value = minxy.lat.toFixed(4);
        }
        if (this.maxyelement) {
           this.maxyelement.dom.value = maxxy.lat.toFixed(4);
        }
