...
{if(type!="pdf")
preparePresent();setSort();var pars=fetchParam('all');pars+=fetchParam('phrase');pars+=fetchParam('or');pars+=fetchParam('without');pars+=fetchParam('title');pars+=fetchParam('abstract');pars+=fetchParam('themekey');pars+=fetchRadioParam('similarity');var region=$('region').value;if(region!="")
...
function toggleMoreFields(){$("all_search_row").toggle();$("phrase_search_row").toggle();$("without_search_row").toggle();var src=$("i_morefields").getAttribute('src');var ndx=src.lastIndexOf('/');src=src.substring(0,ndx+1);if($("all_search_row").visible()==true){$("i_morefields").setAttribute('src',src+'minus.png');}else{$("i_morefields").setAttribute('src',src+'plus.gif');}}
function toggleInspire(){$("inspiresearchfields").toggle();$("arrow_inspire").toggle();var src=$("i_inspire").getAttribute('src');var ndx=src.lastIndexOf('/');src=src.substring(0,ndx+1);if($("inspiresearchfields").visible()==true){$("i_inspire").setAttribute('src',src+'minus.png');}else{$("i_inspire").setAttribute('src',src+'plus.gif');}}
function showInspireSearch(){var inspire=$('inspire');if(inspire.checked){inspire.value='true';}else{inspire.value='';}}
function inspireAnnexChanged(inspireannex){var inspire=$('inspire');if(inspireannex!=""){if(inspire)inspire.checked=true;}else{if(inspire)inspire.checked=false;}}
...
function taggleVisibility(elementId){var element=$(elementId);if(element!=null){if(element.style.display=="none"){element.style.display="block";}
else{element.style.display="none";}}
else{return;}}
function addINSPIREThemes(){var allThemes='';var prefix='&inspiretheme=';var inspireThemeChk=$$('#inspirethemesdiv input[type="checkbox"]');for(i=0;i<inspireThemeChk.length;i++){if(inspireThemeChk[i].checked){allThemes+=prefix+inspireThemeChk[i].value+"*";}}
return allThemes;}
function resetInspireOptions(){$('inspire').checked=false;setParam('title','');setParam('inspireannex','');setParam('inspirebrontype','');setParam('protocol','');setParam('orgselect_inspire','');$('inspire_GeographicalNames').checked=false;$('inspire_AdministrativeUnits').checked=false;$('inspire_Addresses').checked=false;$('inspire_CadastralParcels').checked=false;$('inspire_TransportNetworks').checked=false;$('inspire_Hydrography').checked=false;$('inspire_ProtectedSites').checked=false;$('inspire_Elevation').checked=false;$('inspire_LandCover').checked=false;$('inspire_Orthoimagery').checked=false;$('inspire_Geology').checked=false;$('inspire_StatisticalUnits').checked=false;$('inspire_Buildings').checked=false;$('inspire_Soil').checked=false;$('inspire_LandUse').checked=false;$('inspire_HumanHealthAndSafety').checked=false;$('inspire_UtilityAndGovernmentServices').checked=false;$('inspire_EnvironmentalMonitoringFacilities').checked=false;$('inspire_ProductionAndIndustrialFacilities').checked=false;$('inspire_AgriculturalAndAquacultureFacilities').checked=false;$('inspire_PopulationDistribution-Demography').checked=false;$('inspire_AreaManagementRestrictionRegulationZonesAndReportingUnits').checked=false;$('inspire_NaturalRiskZones').checked=false;$('inspire_AtmosphericConditions').checked=false;$('inspire_MeteorologicalGeographicalFeatures').checked=false;$('inspire_OceanographicGeographicalFeatures').checked=false;$('inspire_SeaRegions').checked=false;$('inspire_Bio-geographicalRegions').checked=false;$('inspire_HabitatsAndBiotopes').checked=false;$('inspire_SpeciesDistribution').checked=false;$('inspire_EnergyResources').checked=false;$('inspire_MineralResources').checked=false;$('inspire_MineralResources').checked=false;$('inspire_MineralResources').checked=false;}