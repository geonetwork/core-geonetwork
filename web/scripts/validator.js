...
Validator.prototype.add = function(rules, parentId)
{
	for (var i=0; i<rules.length; i++)
	{
		...
		else
		{
			...
			rule.ctrl = gn.getElementById($(parentId), rule.id);
		}
		
		if (rule.type == 'length')
			rule.validator = gn.wrap(this, this.lengthVal);
			
		else if (rule.type == 'alphanum')
			rule.validator = gn.wrap(this, this.alphanumVal);
		
		else if (rule.type == 'integer')
			rule.validator = gn.wrap(this, this.integerVal);
			
		else if (rule.type == 'hostname')
			rule.validator = gn.wrap(this, this.hostnameVal);
...
Validator.prototype.showError = function(ctrl, result)
{
	...
	msg = gn.replace(msg, '{NAME}',  name);
	msg = gn.replace(msg, '{VALUE}', value);
