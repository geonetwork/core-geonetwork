...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
       <xsl:template match="/root">
               ...
               <xsl:variable name="op"
                       select="/root/response/myop"></xsl:variable>
               <xsl:choose>
    			...
    			<xsl:when test="$op='scan'">
					...
					<zs:scanResponse xmlns:zs="http://www.loc.gov/zing/srw/">
			    	  ...
			    	  <xsl:if test="response/diagnostics">
					   ...
					   <diagnostic>
			   				<uri><xsl:value-of select="uri"></xsl:value-of></uri>
			   				<message><xsl:value-of select="message"></xsl:value-of></message>
			   				<details><xsl:value-of select="details"></xsl:value-of></details>
			   			</diagnostic>
...
<zs:searchRetrieveResponse xmlns:zs="http://www.loc.gov/zing/srw/">
  <zs:version>1.1</zs:version> 
  <zs:numberOfRecords><xsl:value-of select="$numrec"></xsl:value-of></zs:numberOfRecords> 
  
  <xsl:if test="response/sruresponse/record/*">
  <zs:resultSetId><xsl:value-of select="$id"></xsl:value-of></zs:resultSetId>
  <zs:resultSetIdleTime><xsl:value-of select="$idle"></xsl:value-of></zs:resultSetIdleTime> 
  
  <zs:records>
  <xsl:for-each select="response/sruresponse/record/*">
 <zs:record>
  
  <zs:recordSchema><xsl:value-of select="./*[local-name()='info']/schema"></xsl:value-of></zs:recordSchema> 
  <zs:recordPacking>XML</zs:recordPacking> 
 <zs:recordData>
 <xsl:call-template name="copyrecord">
 </xsl:call-template>
  </zs:recordData>
  <zs:recordPosition><xsl:value-of select="../@recordPosition"></xsl:value-of></zs:recordPosition> 
  </zs:record> 
  </xsl:for-each>
  </zs:records>
  </xsl:if>
                                       ...
                                       <zs:record>
                                               ...
                                               <zs:recordData>
														...
														<serverInfo protocol="SRU" version="1.2"
                                                                       transport="http" method="GET POST">
															<host><xsl:value-of select="$server"/></host>
															<port><xsl:value-of select="$port"></xsl:value-of></port>
                                                        	<database><xsl:value-of select="response/@sruuri"/></database>
                                                        </serverInfo>
                                                        ...
                                                        <indexInfo>
                                                               ...
                                                               <xsl:for-each select="response/sets/set">
                                                                       <set >
                                                                               <xsl:attribute name="name"><xsl:value-of select="@namespace"></xsl:value-of>  </xsl:attribute>
                                                                               <xsl:attribute name="identifier"><xsl:value-of select="@url"></xsl:value-of>  </xsl:attribute>
                                                                       </set>
                                                               </xsl:for-each>
                                                                       ...
                                                                       <index>
                                                                             <xsl:attribute name="id"><xsl:value-of select="@id"/></xsl:attribute>
                                                                       		...
                                                                       		<title><xsl:value-of select="map/@text"/></title>
                                                         ...
                                                         <schemaInfo>
                                                                       <schema name="xml:meta" identifier="http://www.iso.org/iso/catalogue_detail.htm?csnumber=32557">
                                                                               <title>ISO 19139</title>
                                                                       </schema>
                                                         </schemaInfo>
                                                         <configInfo>
                                                                       <default type="numberOfRecords">1</default>
                                                                       <setting type="maximumRecords"><xsl:value-of select="response/@records_per_page"/></setting>
