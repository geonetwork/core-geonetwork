...
<xsl:stylesheet		version="1.0" 
	...
	<xsl:template match="ows:BoundingBox">
		...
		<dc:coverage>North <xsl:value-of select="$north"/>, South <xsl:value-of select="$south"/>, East <xsl:value-of select="$east"/>, West <xsl:value-of select="$west"/>. (Global)</dc:coverage>
