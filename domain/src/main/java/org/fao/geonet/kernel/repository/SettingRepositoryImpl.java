...
import static org.fao.geonet.kernel.repository.SettingRepository.*;
...
public class SettingRepositoryImpl implements SettingRepositoryCustom {
    ...
    public List<Setting> findByPath(String pathToSetting) {
        ...
        if (firstSegment.startsWith(ID_PREFIX)) {
            ...
            TypedQuery<Setting> root = entityManager.createNamedQuery(Setting.QUERY_FIND_ROOT.NAME, Setting.class);
        ...
        for (String childName : pathSegments) {
            ...
            for (Setting setting : oldSettings) {
                TypedQuery<Setting> query = entityManager.createNamedQuery(Setting.QUERY_FIND_CHILDREN_BY_NAME.NAME, Setting.class);
