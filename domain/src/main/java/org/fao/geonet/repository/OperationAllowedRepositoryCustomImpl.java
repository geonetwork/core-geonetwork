...
public class OperationAllowedRepositoryCustomImpl implements OperationAllowedRepositoryCustom {
    ...
    public List<Integer> findAllPublicRecordIds() {
        ...
        Specification<OperationAllowed> spec = where(
            OperationAllowedSpecs.hasOperation(ReservedOperation.view));
        spec = spec.and(OperationAllowedSpecs.hasGroupId(ReservedGroup.all.getId()));

        Specification<Metadata> specIsTemplate = (Specification<Metadata>)isType(MetadataType.METADATA);

        query.where(mdIdEquals,
                    spec.toPredicate(root, query, cb),
                    specIsTemplate.toPredicate(metadataRoot, query, cb));
        query.select(root.get(OperationAllowed_.id).get(OperationAllowedId_.metadataId));
        query.distinct(true);
        return _entityManager.createQuery(query).getResultList();
    }
