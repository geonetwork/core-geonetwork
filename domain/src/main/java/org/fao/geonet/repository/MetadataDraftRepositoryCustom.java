...
/**
 * Custom (Non spring-data) Query methods for {@link MetadataDraft} entities.
 *
 * @author Jesse
 */
public interface MetadataDraftRepositoryCustom {
    /**
     * Return an object that contains functions for calculating several
     * different statistical calculations (related to the metadata) based on the
     ...
     *
     * @param id
     *            the id in string form instead of integer.
     * @return
     */
    @Nullable
    MetadataDraft findOne(@Nonnull String id);
     ...
     * <li><code>new Sort(Sort.Direction.ASC, Metadata_.id.getName())</code>
     * </li>
     * </ul>
     * </p>
     *
     * @param pageable
     *            if non-null then control which subset of the results to return
     *            (and how to sort the results).
     * @return List of &lt;MetadataId, changeDate&gt;
     */
    @Nonnull
    Page<Pair<Integer, ISODate>> findAllIdsAndChangeDates(
     ...
     *
     * @param spec
     *            the specification for identifying the metadata.
     * @return all ids
     */
    @Nonnull
    List<Integer> findAllIdsBy(@Nonnull Specification<MetadataDraft> spec);
     ...
     */
    @Nullable
    MetadataDraft findOneOldestByChangeDate();

    /**
     * Load the source info objects for all the metadata selected by the spec.
     *
     * @param spec
     *            the specification identifying the metadata of interest
     * @return a map of metadataId -> SourceInfo
     */
    Map<Integer, MetadataSourceInfo> findAllSourceInfo(
