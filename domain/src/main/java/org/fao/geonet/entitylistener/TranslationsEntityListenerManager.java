...
/**
 * Created with IntelliJ IDEA. CswCapabilitiesInfoField: Jesse Date: 11/26/13 Time: 1:51 PM
 */
public class TranslationsEntityListenerManager extends AbstractEntityListenerManager<Translations> {
    @PrePersist
    public void prePresist(final Translations entity) {
        handleEvent(PersistentEventType.PrePersist, entity);
    }

    @PreRemove
    public void preRemove(final Translations entity) {
        handleEvent(PersistentEventType.PreRemove, entity);
    }

    @PostPersist
    public void postPersist(final Translations entity) {
        handleEvent(PersistentEventType.PostPersist, entity);
    }

    @PostRemove
    public void postRemove(final Translations entity) {
        handleEvent(PersistentEventType.PostRemove, entity);
    }

    @PreUpdate
    public void preUpdate(final Translations entity) {
        handleEvent(PersistentEventType.PreUpdate, entity);
    }

    @PostUpdate
    public void postUpdate(final Translations entity) {
        handleEvent(PersistentEventType.PostUpdate, entity);
    }

    @PostLoad
    public void postLoad(final Translations entity) {
