...
public class JODAISODateTest extends GeonetworkTestCase {
    ...
    public void testParsegYearMonthDateTime() throws Exception {
        // xs:gYearMonth
        for (int i = 0; i < 10; i++) {
            String year = "20" + getRandom(9, 0) + getRandom(9, 0);
            String month = "0" + getRandom(9, 1);
            String hour = getRandom(1, 0) + getRandom(9, 0);
            String minutes = getRandom(5, 0) + getRandom(9, 0);

            // Format: yyyy-MM-hh:mm Time zone
            String jodaISODate = JODAISODate.parseISODateTime(year + "-"
                    + month + "-" + hour + ":" + minutes + "Z");
            assertEquals(jodaISODate, year + "-" + month + "-01T" + hour + ":"
                    + minutes + ":00.000Z");

            year = "20" + getRandom(9, 0) + getRandom(9, 0);
            month = "0" + getRandom(9, 0);
            // Format: yyyy-MM
            jodaISODate = JODAISODate.parseISODateTime(year + "-" + month);
            assertEquals(jodaISODate, year + "-" + month + "-01T00:00:00.000Z");
        }
    }
