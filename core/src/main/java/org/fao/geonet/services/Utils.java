...
public class Utils {
    ...
    public static String lookupMetadataIdFromFileId(GeonetContext gc, String fileId) throws IOException, SolrServerException {
        SolrSearchManager searchManager = gc.getBean(SolrSearchManager.class);
        return lookupMetadataIdFromFileId(fileId, searchManager);
    }

    public static String lookupMetadataIdFromFileId(String fileId, SolrSearchManager searchManager) throws SolrServerException, IOException {
        SolrDocument doc = searchManager.getDocFieldValue(String.format(
            "+identifier:\"%s\"", fileId), SolrSearchManager.ID);
        if (doc != null) {
            return doc.getFieldValue(SolrSearchManager.ID).toString();
        } else {
            return null;
        }
    }
