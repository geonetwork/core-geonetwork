...
public class SextantLDAPUserDetailsContextMapper extends
	...
	// This does basically the same as AbstractLDAPUserDetailsContextMapper.mapUserFromContext(),
			...
			if ("".equals(privilegePattern)) {
				...
				if (ldapProfiles != null) {
					...
					for (String profile : ldapProfiles) {
						...
						String mappedProfile = profileMapping.get(profile).name();
				...
				ArrayList<String> orgUsr = userInfo.get(mapping.get("organisation")[0]);
	...
	private String getHighestProfile(String[] array) {
		...
		String[] revProfiles = {
				"RegisteredUser", "Editor", "Reviewer", "UserAdmin", "Administrator"
		};
		
		// We hard-code the profile list to avoid introducing too much complexity in the current code
