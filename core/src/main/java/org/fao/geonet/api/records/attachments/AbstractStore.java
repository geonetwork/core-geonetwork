...
public abstract class AbstractStore implements Store {
    ...
    public MetadataResource renameResource(ServiceContext context, String metadataUuid, String resourceId, String newName, Boolean approved) throws Exception {
        ...
        // Resolve the resource by trying the public visibility first and falling back to private
        ResourceHolder resourceHolder = null;
        try {
            try {
                resourceHolder = getResource(context, metadataUuid, MetadataResourceVisibility.PUBLIC, resourceId, approved);
            } catch (ResourceNotFoundException ignored) {
                resourceHolder = getResource(context, metadataUuid, MetadataResourceVisibility.PRIVATE, resourceId, approved);
            }

            try (ResourceHolder rh = resourceHolder) {
                MetadataResource metadataResource = rh.getMetadata();
                ...
                try (InputStream is = rh.getResource().getInputStream()) {
