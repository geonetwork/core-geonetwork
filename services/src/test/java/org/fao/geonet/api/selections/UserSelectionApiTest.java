...
public class UserSelectionApiTest extends UserSelectionApiSupport {
    ...
    public void testUpdateNameSimple() throws Exception {
        ...
        list = update(session, list.getId(),"TESTCASE2", new String[0], UserSelectionApi.ActionType.add);

        assertEquals("TESTCASE2", list.getName());

        //get the list to make sure its fully saved
        list = getList(session, list.getId());
        assertEquals("TESTCASE2", list.getName());
    }
    ...
    public void changeStatus() throws Exception {
        ...
        list1 = setstatus(session,list1.getId(), true);
        assertEquals(true, list1.getIsPublic());

        list1 = setstatus(session,list1.getId(), false);
        assertEquals(false, list1.getIsPublic());
    }
