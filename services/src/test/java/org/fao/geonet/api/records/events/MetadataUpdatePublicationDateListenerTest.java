...
public class MetadataUpdatePublicationDateListenerTest {

    private MetadataUpdatePublicationDateListener listener;
    ...
    public void setUp() throws Exception {
        listener = new MetadataUpdatePublicationDateListener();
    ...
    private void setField(String name, Object value) throws Exception {
        Field field = MetadataUpdatePublicationDateListener.class.getDeclaredField(name);
    ...
    public void doesNothingWhenSettingDisabled() {
        ...
        listener.onApplicationEvent(new MetadataPublished(md));

        verify(service, never()).addPublicationDate(any(), any());
    }
    ...
    @Test
    public void setsPublicationDateForNormalMetadata() {
        when(settingManager.getValueAsBool(Settings.SYSTEM_METADATAPRIVS_PUBLICATION_MANAGEPUBLICATIONDATE)).thenReturn(true);

        Metadata md = metadataWithType(MetadataType.METADATA, 1, "uuid-1");
        listener.onApplicationEvent(new MetadataPublished(md));

        verify(service).addPublicationDate(eq(md), any(ISODate.class));
    }
    ...
    public void resolvesApprovedRecordWhenEventCarriesADraft() {
        ...
        listener.onApplicationEvent(new MetadataPublished(draft));

        // The approved record must be used, not the draft itself.
        verify(service).addPublicationDate(eq(approved), any(ISODate.class));
    }
    ...
    public void serviceFailureDoesNotPropagate() {
        ...
        Metadata md = metadataWithType(MetadataType.METADATA, 1, "uuid-1");
        org.mockito.Mockito.doThrow(new RuntimeException("boom")).when(service).addPublicationDate(any(), any());

        // Must not throw: a failure updating the publication date must not break the publish action.
        listener.onApplicationEvent(new MetadataPublished(md));
    }
}
