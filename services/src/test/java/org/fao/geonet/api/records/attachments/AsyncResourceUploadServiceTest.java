...
public class AsyncResourceUploadServiceTest {
    ...
    public void submissionPersistsPendingTaskAndExecutesImmediately()
        ...
        assertTrue(localExecutions().containsKey(result.getId()));
    }
    ...
    /**
     * Verifies that credentials embedded in the source URL are used only by
     * the local worker and are not persisted in the task.
     */
    @Test
    public void submissionSanitizesPersistedSourceUrl() throws Exception {
        ResourceUploadTask result = service.submit(
            store,
            requestContext,
            METADATA_UUID,
            new URL(
                "https://user:password@example.org/file%20name.zip" +
        ...
        assertEquals(
            "https://example.org/file%20name.zip",
            result.getSourceUrl()
        );
    }
    ...
    public void submissionIsDeferredUntilTransactionCommits()
        ...
        verify(executor).execute(any(FutureTask.class));
        assertTrue(localExecutions().containsKey(task.getId()));
    }

    /**
     * Verifies that a zero-row progress update does not stop an execution that
     * merely moved between active states, while cancellation, terminal state,
     * deletion, and worker reassignment do stop it.
     */
    @Test
    public void localExecutionIsCancelledOnlyForInactivePersistentTask() {
        String taskId = "task-id";

        when(repository.findStatusByIdAndWorkerId(
            eq(taskId),
            anyString()
        )).thenReturn(
            Optional.of(ResourceUploadTaskStatus.UPLOADING),
            Optional.of(ResourceUploadTaskStatus.FINALIZING),
            Optional.of(ResourceUploadTaskStatus.CANCELLING),
            Optional.of(ResourceUploadTaskStatus.COMPLETED),
            Optional.empty()
        );

        assertFalse(shouldCancelLocalExecution(taskId));
        assertFalse(shouldCancelLocalExecution(taskId));
        assertTrue(shouldCancelLocalExecution(taskId));
        assertTrue(shouldCancelLocalExecution(taskId));
        assertTrue(shouldCancelLocalExecution(taskId));
    }

    /**
     * Verifies that the periodic synchronizer observes a cancellation request
     * written directly to persistent state and cancels the local execution.
     */
    @Test
    public void synchronizerCancelsExecutionRequestedThroughPersistentState()
        throws Throwable {

        String taskId = "task-id";
        String workerId = (String) ReflectionTestUtils.getField(
            service,
            "workerId"
        );
        ...
        localExecutions().put(taskId, execution);
        ...
        when(repository.findIdsByWorkerIdAndStatus(
            workerId,
        ...
        try (
            MockedStatic<ApplicationContextHolder> applicationContext =
                mockStatic(ApplicationContextHolder.class);
            MockedStatic<TransactionManager> transactionManager =
                mockStatic(TransactionManager.class)
        ) {
            applicationContext.when(ApplicationContextHolder::get)
                .thenReturn(appContext);

            transactionManager.when(() ->
                TransactionManager.<Set<String>>runInTransaction(
                    eq("SynchronizeResourceUploadTasks"),
                    same(appContext),
                    eq(TransactionManager.TransactionRequirement.CREATE_NEW),
                    eq(TransactionManager.CommitBehavior.ALWAYS_COMMIT),
                    eq(false),
                    any()
                )
            ).thenAnswer(invocation -> {
                TransactionTask<Set<String>> task =
                    invocation.getArgument(5);
                return task.doInTransaction(null);
            });
        ...
        verify(repository).updateHeartbeat(
            eq(taskId),
            eq(workerId),
            eq(ResourceUploadTaskStatus.PENDING),
            any()
    ...
    /**
     * Invokes the service's persistent-state check used after a conditional
     * heartbeat or progress update affects no rows.
     */
