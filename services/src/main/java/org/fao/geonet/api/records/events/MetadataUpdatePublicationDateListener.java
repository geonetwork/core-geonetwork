...
public class MetadataUpdatePublicationDateListener implements ApplicationListener<MetadataPublished> {
    ...
    public void onApplicationEvent(MetadataPublished event) {
        ...
        // version. The publication date must be set on the approved record, not the draft.
        ...
        try {
            metadataUpdatePublicationDateService.addPublicationDate(md, new ISODate());
        } catch (Exception e) {
            Log.error("org.fao.geonet.services.metadata", "Error setting publication date of metadata " + md.getId(), e);
        }
    }
