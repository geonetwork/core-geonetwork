                ...
                for (AbstractMetadata record : metadataRepository.findAllByUuid(uuid)) {
                    ...
                    if (approved == null ||
                        ...
                        if (!accessMan.canEdit(serviceContext, String.valueOf(record.getId()))) {
                            ...
                            final Pair<Element, String> validationPair = validator.doValidate(userSession, record.getDataInfo().getSchemaId(), Integer.toString(record.getId()), xmlSerializer.select(serviceContext, String.valueOf(record.getId())),
                                serviceContext.getLanguage(), false);
