 ...
 * metadata.show service to retrieve the xml of the metadata. You can specify the HTTP one, with the
 * param "url" which will call an HTTP request to retrieve external xml of a metadata.
 *
 * @author fgravin
 */
public enum Loader {
...
//     * HTTP Enumeration
//     *
//     * Used to retrieve the xml of the metadata through an external http request. Needs the url
//     * parameter in the formatter url (should be encoded): http://localhost:8080/geonetwork/srv/fre/metadata.formatter.html?xsl=default&loader=HTTP&url=http%3A%2F%2Flocalhost%3A8080%2Fgeonetwork%2Fsrv%2Ffre%2Fcsw%3FSERVICE%3DCSW%26VERSION%3D2.0.2%26REQUEST%3DGetRecordById%26ID%3D43484350-02cf-11e0-abec-005056987263%26outputschema%3Dcsw%3AIsoRecord%26ElementSetName%3Dfull
//     *
//     * The url we be called and should return a valid metadata XML. You can specify which element
//     * from this result that will be inserted in the formatter output with the config properties
//     * loader.http. For example, GetRecordById response will gives <csw:GetRecordByIdResponse><gmd:MD_Metadata>
//     * while a metadata.show will return directly the <gmd:MD_Metadata> element as root.
//     * loader.http=* value will outpass the <csw:GetRecordByIdResponse> and insert <gmd:MD_Metadata>
//     * under the root element of the formatter output.
//     */
//    HTTP {
//
//        /**
...
//
//    /**
//     * SHOW enumeration Will be called by default if no or not valid loader param is specified. Show
//     * will call internal metadata.show service to retrieve the xmlo of the metadata. Need uuid or
//     * id param in the url.
//     */
//    SHOW {
//
...
//     * Loader.HTTP will check if the http url is not on the same host as the formatter, in this
//     * case, it will use internal service through Loader.SHOW This process can be activated or not
//     * with the config property loader.redirect
//     *
...
//        catch (Exception e) {
//            throw new IllegalArgumentException(name + " is not a valid loader for the formatter");
//        }
//        return l;
//    }
//
