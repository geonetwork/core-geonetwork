...
public class LastMonthSummary extends NotInReadOnlyModeService {
	...
	public Element serviceSpecificExec(Element params, ServiceContext context) throws Exception {
        ...
        List<Element> resultSet = dbms.select(query, monthAgo).getChildren();
		if (resultSet.size() > 2) {
            if(Log.isDebugEnabled(Geonet.SEARCH_LOGGER))
        ...
        if (resultSet.size() == 1) {
            ...
            if (((Element)resultSet.get(0)).getChildText("simple").equals("0")) {
                advancedSearchElement.setText(resultSet.get(0).getChildText("cnt"));
            } else {
                simpleSearchElement.setText(resultSet.get(0).getChildText("cnt"));

            }
        } else if (resultSet.size() == 2) {
            advancedSearchElement.setText(resultSet.get(0).getChildText("cnt"));
            simpleSearchElement.setText(resultSet.get(1).getChildText("cnt"));
        }
