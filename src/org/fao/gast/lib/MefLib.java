...
public class MefLib
{
	...
	public void doImport(final Dbms dbms, final int id, File mefFile) throws Exception
	{
		...
		MEFLib.visit(mefFile, new MEFFileVisitor(), new MEFVisitor()
		{
			...
			public void handlePrivateFile(String file, String changeDate, InputStream is) throws IOException
			{
	...
	private void addMetadata(Dbms dbms, int id, Element md ,Element info) throws Exception
	{
		int owner = 1;
		...
		Lib.log.debug(" - Adding XML with uuid : "+ uuid);

		Lib.metadata.insertMetadata(dbms, schema, md, id, source, createDate, changeDate, uuid,
											  owner, groupOwner, isTemplate, null);

		addCategories(dbms, id, info.getChild("categories"));
		addPrivileges(dbms, id, info.getChild("privileges"));

		//TODO: index metadata here
		//index is performed at startup
		//indexMetadata(dbms, id);
	}

	//--------------------------------------------------------------------------

	private void saveFile(int id, String file, String changeDate, InputStream is, String access) throws IOException
	{
		...
		Lib.log.debug("   - Destination folder is : "+ outDir.getAbsolutePath());
		outDir.mkdirs();
	...
	private void addCategories(Dbms dbms, int id, Element categ) throws Exception
	{
	...
	private void addPrivileges(Dbms dbms, int id, Element privil) throws Exception
	{
	...
	private void addOperations(Dbms dbms, Element group, int id, int grpId) throws Exception
	{
