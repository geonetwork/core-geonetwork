...
-->
	...
	<sch:pattern name="$loc/strings/M8">
		<sch:rule context="//gmd:CI_ResponsibleParty">
			<sch:assert
				test="(count(gmd:individualName[@gco:nilReason!='missing' or not(@gco:nilReason)]) 
				+ count(gmd:organisationName[@gco:nilReason!='missing' or not(@gco:nilReason)])
				+ count(gmd:positionName[@gco:nilReason!='missing' or not(@gco:nilReason)])) > 0"
	...
	<sch:pattern name="$loc/strings/M9">
		<sch:rule context="//gmd:MD_LegalConstraints|//*[@gco:isoType='gmd:MD_LegalConstraints']">
			<sch:report
				test="gmd:accessConstraints/gmd:MD_RestrictionCode/@codeListValue='otherRestrictions' and (not(gmd:otherConstraints) or gmd:otherConstraints/@gco:nilReason='missing')"
				>$loc/strings/alert.M9.access</sch:report>
			<sch:report
				test="gmd:useConstraints/gmd:MD_RestrictionCode/@codeListValue='otherRestrictions' and (not(gmd:otherConstraints) or gmd:otherConstraints/@gco:nilReason='missing')"
				>$loc/strings/alert.M9.use</sch:report>
		</sch:rule>
	</sch:pattern>
	...
	<sch:pattern name="$loc/strings/M10">
		<sch:rule context="//gmd:MD_Band">
			<sch:report test="(gmd:maxValue[@gco:nilReason!='missing' or not(@gco:nilReason)]
				or gmd:minValue[@gco:nilReason!='missing' or not(@gco:nilReason)]) 
				and not(gmd:units)"
				>$loc/strings/alert.M9</sch:report>
	...
	<sch:pattern name="$loc/strings/M11">
		<sch:rule context="//gmd:LI_Source">
			<sch:assert test="gmd:description[@gco:nilReason!='missing' or not(@gco:nilReason)] or gmd:sourceExtent"
	...
	<sch:pattern name="$loc/strings/M13">
		<sch:rule context="//gmd:DQ_DataQuality">
			<sch:report
				test="(((count(*/gmd:LI_Lineage/gmd:source) + count(*/gmd:LI_Lineage/gmd:processStep)) = 0) 
					and (gmd:scope/gmd:DQ_Scope/gmd:level/gmd:MD_ScopeCode/@codeListValue='dataset' 
							or gmd:scope/gmd:DQ_Scope/gmd:level/gmd:MD_ScopeCode/@codeListValue='series')) 
					and not(gmd:lineage/gmd:LI_Lineage/gmd:statement) 
					"
				>$loc/strings/alert.M13</sch:report>
		</sch:rule>
	</sch:pattern>
	<!-- TEST  8 FXCHECK -->
	<sch:pattern name="$loc/strings/M14">
		<sch:rule context="//gmd:LI_Lineage">
			<sch:report test="not(gmd:source) 
				and not(gmd:statement[@gco:nilReason!='missing' or not(@gco:nilReason)]) 
				and not(gmd:processStep)"
				>$loc/strings/alert.M14</sch:report>
		</sch:rule>
	</sch:pattern>
	<!-- TEST  9 FXCHECK -->
	<sch:pattern name="$loc/strings/M15">
		<sch:rule context="//gmd:LI_Lineage">
			<sch:report test="not(gmd:processStep) 
				and not(gmd:statement[@gco:nilReason!='missing' or not(@gco:nilReason)]) 
				and not(gmd:source)"
				>$loc/strings/alert.M15</sch:report>
		</sch:rule>
	</sch:pattern>
	<!-- TEST 5 FXCHECK -->
	<sch:pattern name="$loc/strings/M16">
		<sch:rule context="//gmd:DQ_DataQuality">
			<sch:report
				test="gmd:scope/gmd:DQ_Scope/gmd:level/gmd:MD_ScopeCode/@codeListValue='dataset' 
								and not(gmd:report) 
								and not(gmd:lineage)"
				>$loc/strings/alert.M16</sch:report>
		</sch:rule>
	</sch:pattern>
	<!-- TEST  6 FXCHECK -->
	<sch:pattern name="$loc/strings/M17">
		<sch:rule context="//gmd:DQ_Scope">
			<sch:assert
				test="gmd:level/gmd:MD_ScopeCode/@codeListValue='dataset' 
	...
	<sch:pattern name="$loc/strings/M18">
		<sch:rule context="//gmd:MD_Medium">
			<sch:report test="gmd:density and not(gmd:densityUnits[@gco:nilReason!='missing' or not(@gco:nilReason)])"
				>$loc/strings/alert.M18</sch:report>
		</sch:rule>
	</sch:pattern>
	<!-- TEST15 FXCHECK -->
	<sch:pattern name="$loc/strings/M19">
		<sch:rule context="//gmd:MD_Distribution">
			<sch:assert
				test="count(gmd:distributionFormat)>0 
	...
	<sch:pattern name="$loc/strings/M20">
		<sch:rule context="//gmd:EX_Extent">
			<sch:assert
				test="count(gmd:description[@gco:nilReason!='missing' or not(@gco:nilReason)])>0 
	...
	<sch:pattern name="$loc/strings/M21">
		<sch:rule context="//gmd:MD_DataIdentification|//*[@gco:isoType='gmd:MD_DataIdentification']">
			<sch:report
				...
				>$loc/strings/alert.M21</sch:report>
		</sch:rule>
	</sch:pattern>
	<!-- TEST  2 -->
	<sch:pattern name="$loc/strings/M22">
		<sch:rule context="//gmd:MD_DataIdentification|//*[@gco:isoType='gmd:MD_DataIdentification']">
			<sch:report
				...
				>$loc/strings/alert.M6</sch:report>
		</sch:rule>
	</sch:pattern>
	<!-- TEST  3 -->
	...
	<sch:pattern name="$loc/strings/M26">
		<sch:rule context="//gmd:MD_ExtendedElementInformation">
			...
			<sch:assert
				...
				>$loc/strings/alert.M26.minimumOccurence</sch:assert>
			<sch:assert
				test="(gmd:dataType/gmd:MD_DatatypeCode/@codeListValue='codelist' 
				or gmd:dataType/gmd:MD_DatatypeCode/@codeListValue='enumeration' 
				or gmd:dataType/gmd:MD_DatatypeCode/@codeListValue='codelistElement') 
	...
	<sch:pattern name="$loc/strings/M27">
		<sch:rule context="//gmd:MD_ExtendedElementInformation">
			<sch:report
				...
				>$loc/strings/alert.M27</sch:report>
		</sch:rule>
	</sch:pattern>
	<!-- TEST 18 -->
	<sch:pattern name="$loc/strings/M28">
		<sch:rule context="//gmd:MD_ExtendedElementInformation">
			<sch:report
				...
				>$loc/strings/alert.M28</sch:report>
		</sch:rule>
	</sch:pattern>
	<!-- TEST 19 -->
	<sch:pattern name="$loc/strings/M29">
		<sch:rule context="//gmd:MD_ExtendedElementInformation">
			<sch:report
				...
				>$loc/strings/alert.M29</sch:report>
		</sch:rule>
	</sch:pattern>
	...
	<sch:pattern name="$loc/strings/M30">
		<sch:rule context="//gmd:MD_Georectified">
			<sch:report
				...
				>$loc/strings/alert.M30</sch:report>
		</sch:rule>
	</sch:pattern>
	<!--  -->
	<sch:pattern name="$loc/strings/M61">
		<sch:rule context="//gmd:MD_Metadata/gmd:hierarchyLevel|//*[@gco:isoType='gmd:MD_Metadata']/gmd:hierarchyLevel">
			<sch:report
				test="(gmd:MD_ScopeCode/@codeListValue!='' and gmd:MD_ScopeCode/@codeListValue!='dataset') and 
				(not(../gmd:hierarchyLevelName) or ../gmd:hierarchyLevelName/@gco:nilReason)"
				>$loc/strings/alert.M61</sch:report>
		</sch:rule>
	</sch:pattern>
